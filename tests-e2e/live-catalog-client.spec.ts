...
test.describe('Live Catalog - Client Workflows', () => {
  ...
  test('should browse catalog with filters', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Live Catalog' })).toBeVisible();
  ...
  test('should add items to interest list', async ({ page }) => {
    ...
    // Add second item
    await page.getByTestId('product-card').nth(1).getByRole('button', { name: 'Add to Interest' }).click();
  ...
  test('should view and manage interest list draft', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Interest List' })).toBeVisible();
  ...
  test('should submit interest list successfully', async ({ page }) => {
    ...
    await expect(page.getByText(/Interest list submitted successfully/i)).toBeVisible();
  ...
  test('should show change indicators for price changes', async ({ page }) => {
    ...
    const changeIndicators = await page.getByTestId('price-change-indicator').count();
  ...
  test('should create price alert for product', async ({ page }) => {
    ...
    await page.getByTestId('product-card').first().getByRole('button', { name: 'Set Price Alert' }).click();
    ...
    // Verify success message
    await expect(page.getByText(/Price alert created successfully/i)).toBeVisible();

    // Verify alert appears in My Price Alerts section
    await page.getByRole('heading', { name: 'My Price Alerts' }).scrollIntoViewIfNeeded();
