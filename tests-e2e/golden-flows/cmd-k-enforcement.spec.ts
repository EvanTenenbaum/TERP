...
test.describe("UXS-603: Command Palette Scope Enforcement", () => {
  ...
  test.describe("Cmd+K Focus Behavior", () => {
    test("Cmd+K should focus search input on list pages", async ({ page }) => {
      ...
      const pageSearchFocused = pageSearchVisible
        ? await pageSearchInput.evaluate(
    ...
    test(
      "Cmd+K should focus search input on pick-pack page",
      async ({ page }) => {
        ...
        if (hasPickPackSearch) {
          ...
          const paletteInput = page
            .locator("[cmdk-input]")
            .or(
              page.locator('input[placeholder*="command or search" i]')
            );
          const paletteVisible = await paletteInput
            .first()
            .isVisible()
  ...
  test.describe("Command Palette Actions", () => {
    test(
      "command palette should offer navigation commands",
      async ({ page }) => {
        await page.goto("/");
        await page.waitForLoadState("networkidle");
    ...
    test(
      "command palette should NOT offer data entry",
      async ({ page }) => {
        await page.goto("/");
        await page.waitForLoadState("networkidle");
  ...
  test.describe("Search vs Data Entry Distinction", () => {
    ...
    test(
      "Cmd+K on form pages should NOT auto-fill fields",
      async ({ page }) => {
        await page.goto("/orders/create");
        await page.waitForLoadState("networkidle");
  ...
  test.describe("Context-Aware Behavior", () => {
    test(
      "Cmd+K behavior should be consistent across pages",
      async ({ page }) => {
        const pages = [
          "/orders",
          "/inventory",
