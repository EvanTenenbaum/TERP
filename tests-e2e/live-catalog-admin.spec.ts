...
test.describe('Live Catalog - Admin Workflows', () => {
  ...
  test('should view interest list details with change detection', async ({ page }) => {
    ...
    await page.getByRole('row').nth(1).getByRole('button', { name: 'View Details' }).click();

    // Verify modal is visible
    await expect(page.getByRole('dialog', { name: 'Interest List Details' })).toBeVisible();
  ...
  test('should view and manage price alerts', async ({ page }) => {
    ...
    if (deactivateButtons > 0) {
      ...
      // Verify success message
      await expect(page.getByText(/Alert deactivated successfully/i)).toBeVisible();
    }
  });
  ...
  test('should support partial item selection for orders', async ({ page }) => {
    ...
    const selectedCount = await page.getByTestId('selected-items-count').textContent();
