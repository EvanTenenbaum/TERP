...
test.describe('Navigation and UI Interactions', () => {
  ...
  test('should toggle sidebar collapse', async ({ page }) => {
    ...
    const toggleButton = page.locator('button[aria-label*="sidebar" i], button[aria-label*="menu" i]').first();
  ...
  test('should display user menu', async ({ page }) => {
    ...
    const userMenu = page.locator('button[aria-label*="user" i], button[aria-label*="account" i], [data-user-menu]').first();
    
    if (await userMenu.isVisible()) {
      await userMenu.click();
      await expect(page.locator('[role="menu"], .dropdown-menu').first()).toBeVisible();
    }
  });
  ...
  test('should search globally', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const results = page.locator('[role="listbox"], .search-results, [data-search-results]').first();
  ...
  test('should display notifications', async ({ page }) => {
    ...
    const notificationButton = page.locator('button[aria-label*="notification" i], [data-notifications]').first();
    
    if (await notificationButton.isVisible()) {
      await notificationButton.click();
      await expect(page.locator('[role="menu"], .notifications-panel').first()).toBeVisible();
    }
  });
  ...
  test('should navigate using breadcrumbs', async ({ page }) => {
    ...
    if (await firstRow.isVisible()) {
      ...
      const breadcrumbs = page.locator('nav[aria-label*="breadcrumb" i], .breadcrumb').first();
  ...
  test('should handle modal dialogs', async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("Add"), button:has-text("New")').first();
