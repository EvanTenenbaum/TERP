...
test.describe('Inventory CRUD Operations', () => {
  ...
  test('should display inventory table', async ({ page }) => {
    await page.goto('/inventory');
    await page.waitForSelector('table, [role="table"], .inventory-grid', { timeout: 5000 });
  ...
  test('should search inventory items', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      await expect(page.locator('tbody tr, [role="row"], .inventory-item').first()).toBeVisible();
    }
  });
  ...
  test('should filter inventory by category', async ({ page }) => {
    ...
    const filterButton = page.locator('button:has-text("Filter"), select[name*="category"]').first();
  ...
  test('should view inventory item details', async ({ page }) => {
    await page.goto('/inventory');
    const firstItem = page.locator('tbody tr, [role="row"], .inventory-item').first();
    await firstItem.click();
    
    // Should show details modal or navigate to detail page
    await expect(page.locator('[role="dialog"], .modal').first().or(page)).toHaveURL(/\/inventory\/\d+/);
  });
  ...
  test('should create new inventory item', async ({ page }) => {
    ...
    await page.fill('input[name="name"], input[placeholder*="name" i]', 'Test Product E2E');
    await page.fill('input[name="sku"], input[placeholder*="sku" i]', 'TEST-SKU-001');
    ...
    await expect(page.locator('.toast, [role="alert"]').filter({ hasText: /success|created/i })).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should adjust inventory quantity', async ({ page }) => {
    ...
    const adjustButton = page.locator('button:has-text("Adjust"), button:has-text("Update Quantity")').first();
    ...
    if (await adjustButton.isVisible()) {
      ...
      await expect(page.locator('.toast, [role="alert"]').filter({ hasText: /success|updated/i })).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should show low stock items', async ({ page }) => {
    ...
    const lowStockFilter = page.locator('button:has-text("Low Stock"), [data-filter="low-stock"]').first();
  ...
  test('should view inventory movement history', async ({ page }) => {
    ...
    if (await historyTab.isVisible()) {
      ...
      await expect(page.locator('.history-item, tbody tr').first()).toBeVisible({ timeout: 5000 });
    }
  });
});
