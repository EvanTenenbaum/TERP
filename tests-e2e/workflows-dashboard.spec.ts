...
test.describe('Workflows and Dashboard', () => {
  ...
  test('should display workflow items', async ({ page }) => {
    ...
    const workflowItems = page.locator('.workflow-item, [role="listitem"], tbody tr');
  ...
  test('should complete a workflow task', async ({ page }) => {
    ...
    const firstItem = page.locator('.workflow-item, [role="listitem"], tbody tr').first();
    
    if (await firstItem.isVisible()) {
      await firstItem.click();
      
      const completeButton = page.locator('button:has-text("Complete"), button:has-text("Done")').first();
      
      if (await completeButton.isVisible()) {
        await completeButton.click();
        await expect(page.locator('.toast, [role="alert"]').filter({ hasText: /success|completed/i })).toBeVisible({ timeout: 5000 });
      }
    }
  });
  ...
  test('should navigate between dashboard sections', async ({ page }) => {
    ...
    const inventoryLink = page.locator('a[href="/inventory"], nav a:has-text("Inventory")').first();
  ...
  test('should display recent activity', async ({ page }) => {
    ...
    const activitySection = page.locator('[data-section="activity"], .activity, h3:has-text("Activity")').first();
  ...
  test('should refresh dashboard data', async ({ page }) => {
    ...
    if (await refreshButton.isVisible()) {
      ...
      await expect(page.locator('[data-widget], .widget').first()).toBeVisible();
    }
  });
