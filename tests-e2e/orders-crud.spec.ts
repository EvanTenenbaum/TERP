...
test.describe('Orders CRUD Operations', () => {
  ...
  test('should navigate to orders page', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /order/i })).toBeVisible();
  });
  ...
  test('should open create order page', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"], .modal').first().or(page.locator('h1:has-text("Create Order")'))).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should view order details', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"], .modal, h1:has-text("Order")').first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should sort orders by date', async ({ page }) => {
    ...
    const dateHeader = page.locator('th:has-text("Date"), [role="columnheader"]:has-text("Date")').first();
  ...
  test('should update order status', async ({ page }) => {
    ...
    const statusDropdown = page.locator('select[name*="status"], button:has-text("Status")').first();
  ...
  test('should add items to order', async ({ page }) => {
    ...
    if (await addItemButton.isVisible()) {
      ...
      await expect(page.locator('table tbody tr, .line-item').first()).toBeVisible();
    }
  });
