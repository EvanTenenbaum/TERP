...
export async function loginAsVipClient(
  page: import("@playwright/test").Page
): Promise<void> {
  ...
  await fillFirstVisible(
    ...
    TEST_USERS.vipClient.email
  );
  await fillFirstVisible(
    page,
    ['input[name="password"]', "#password", 'input[type="password"]'],
    TEST_USERS.vipClient.password
  );
  await page.click('button[type="submit"]');
...
export const authenticatedTest = base.extend({
  storageState: async ({ _dummy }, runTest) => {
    // This would normally point to a saved storage state file
    // For now, we'll just use the base test
