...
test.describe("Calendar Page @prod-regression", () => {
  ...
  test("should display current month", async ({ page }) => {
    ...
    await expect(
      page.locator(`text=/${monthRegex.source}/i`).first()
    ).toBeVisible();
  });
  ...
  test("should display calendar grid", async ({ page }) => {
    ...
    const dayHeader = page
      .locator("text=/sun|mon|tue|wed|thu|fri|sat/i")
      .first();
    await expect(dayHeader).toBeVisible();
  });
});

test.describe("Calendar View Switching @prod-regression", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });
  ...
  test("should have view switcher buttons", async ({ page }) => {
    ...
    const viewButtons = page.locator(
      'button:has-text("Month"), button:has-text("Week"), button:has-text("Day"), button:has-text("Agenda")'
    );
...
test.describe("Calendar Navigation @prod-regression", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });
...
test.describe("Calendar Events @dev-only", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });
