...
test.describe("Feature Flags Settings @prod-regression @feature-flag", () => {
  ...
  test("should display Feature Flags tab", async ({ page }) => {
    ...
    const featureFlagsTab = page
      .locator('text=Feature Flags, [data-testid="feature-flags-tab"]')
      .first();
  ...
  test("should show feature flags list when clicking tab", async ({ page }) => {
    ...
    const flagsContent = page
      .locator("text=/flag|feature|enable|disable/i")
      .first();
  ...
  test("should have Open Feature Flags Manager button", async ({ page }) => {
    ...
    const managerButton = page
      .locator(
        'button:has-text("Open"), button:has-text("Manager"), button:has-text("Manage")'
      )
      .first();
...
test.describe("Feature Flags Manager @dev-only @feature-flag", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsAdmin(page);
  });
  ...
  test("should open feature flags manager dialog", async ({ page }) => {
    ...
    const managerButton = page
      .locator(
        'button:has-text("Open Feature Flags Manager"), button:has-text("Manage Flags")'
      )
      .first();
  ...
  test("should display list of feature flags", async ({ page }) => {
    ...
    const managerButton = page
      .locator(
        'button:has-text("Open Feature Flags Manager"), button:has-text("Manage")'
      )
      .first();
    if (await managerButton.isVisible()) {
      await managerButton.click();
      ...
      const flagToggle = page
        .locator(
          '[role="switch"], input[type="checkbox"], [data-testid="flag-toggle"]'
        )
        .first();
  ...
  test("should have toggle switches for each flag", async ({ page }) => {
    ...
    const managerButton = page
      .locator('button:has-text("Open"), button:has-text("Manager")')
      .first();
    if (await managerButton.isVisible()) {
      await managerButton.click();
      await page.waitForLoadState("networkidle");
...
test.describe("VIP Access Settings @prod-regression", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsAdmin(page);
  });
  ...
  test("should display VIP Access tab", async ({ page }) => {
    ...
    const vipAccessTab = page
      .locator('text=VIP Access, [data-testid="vip-access-tab"]')
      .first();
  ...
  test("should show session list in Active Sessions tab", async ({ page }) => {
    ...
    if (await activeSessionsTab.isVisible()) {
      ...
      const sessionContent = page
        .locator("table, text=/no active|session/i")
        .first();
  ...
  test("should display Audit History tab", async ({ page }) => {
    ...
    const auditHistoryTab = page
      .locator("text=Audit History, text=History")
      .first();
