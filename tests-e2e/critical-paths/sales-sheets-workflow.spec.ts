...
test.describe("Sales Sheets Page @prod-regression", () => {
  ...
  test("should display Sales Sheet Creator title", async ({ page }) => {
    ...
    const title = page
      .locator('h1:has-text("Sales"), text=/sales sheet/i')
      .first();
  ...
  test("should have client selector", async ({ page }) => {
    ...
    const clientSelector = page
      .locator(
        'text=/select.*client|choose.*client/i, [data-testid="client-selector"], select'
      )
      .first();
...
test.describe("Sales Sheet Client Selection @prod-regression", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });
  ...
  test("should open client dropdown", async ({ page }) => {
    ...
    const clientSelector = page
      .locator(
        '[data-testid="client-selector"], select, button:has-text("Select Client")'
      )
      .first();
    ...
    if (await clientSelector.isVisible()) {
      ...
      const options = page
        .locator('[role="option"], option, [data-testid="client-option"]')
        .first();
  ...
  test("should select a client from dropdown", async ({ page }) => {
    ...
    const clientSelector = page
      .locator(
        '[data-testid="client-selector"], select, button:has-text("Select")'
      )
      .first();
  ...
  test("should show inventory browser after client selection", async ({
    ...
    if (await clientSelector.isVisible()) {
      ...
      if (await firstOption.isVisible()) {
        ...
        const inventoryBrowser = page
          .locator("text=/inventory|product|item|browse/i")
          .first();
...
test.describe("Sales Sheet Inventory Browser @prod-regression", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });
  ...
  test("should display inventory items", async ({ page }) => {
    ...
    const inventoryList = page
      .locator('table, [data-testid="inventory-grid"]')
      .first();
  ...
  test("should filter inventory by category", async ({ page }) => {
    ...
    const categoryFilter = page
      .locator(
        '[data-testid="category-filter"], select[name="category"], text=/filter.*category/i'
      )
      .first();
    ...
    if (await categoryFilter.isVisible()) {
      ...
      const categoryOption = page
        .locator(
          '[role="option"]:has-text("Flower"), option:has-text("Flower")'
        )
        .first();
...
test.describe("Sales Sheet Draft Management @prod-regression", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });
  ...
  test("should have Load Draft option", async ({ page }) => {
    ...
    const loadDraftButton = page
      .locator(
        'button:has-text("Load Draft"), button:has-text("Load"), text=/draft/i'
      )
      .first();
  ...
  test("should have Export/Generate button", async ({ page }) => {
    ...
    const exportButton = page
      .locator(
        'button:has-text("Export"), button:has-text("Generate"), button:has-text("PDF")'
      )
      .first();
