...
test.describe("Locations Page", () => {
  ...
  test("should display location hierarchy", async ({ page }) => {
    ...
    const locationData = page.locator('text=/site|zone|rack|shelf|bin/i').first();
  ...
  test("should have expandable location tree", async ({ page }) => {
    ...
    const expandButton = page.locator('[data-testid="expand-location"], button:has-text("+"), [aria-expanded]').first();
    ...
    if (await expandButton.isVisible().catch(() => false)) {
      ...
      const childLocations = page.locator('[data-level="1"], .child-location, .nested-location').first();
      // Tree should have expanded
      expect(await expandButton.getAttribute('aria-expanded') === 'true' || await childLocations.isVisible().catch(() => false)).toBeTruthy();
    }
  });
...
test.describe("Location CRUD Operations", () => {
  ...
  test("should display location details on click", async ({ page }) => {
    ...
    const locationRow = page.locator('tbody tr, [data-testid="location-row"]').first();
    
    if (await locationRow.isVisible().catch(() => false)) {
      await locationRow.click();
      
      // Should show details panel or navigate to detail page
      const details = page.locator('[role="dialog"], .location-details, [data-testid="location-detail"]').first();
