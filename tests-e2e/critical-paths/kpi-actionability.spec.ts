...
test.describe("Dashboard KPI Actionability", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });

  test("should navigate to invoices when clicking Cash Collected", async ({ page }) => {
    await page.goto("/");
  ...
  test("should navigate to client profile when clicking Sales Top 10 row", async ({ page }) => {
    await page.goto("/");
    
    // Find the Sales Top 10 widget and click a client row
    const clientRow = page.locator('[data-testid="sales-top-10"] tbody tr, .sales-widget tbody tr').first();
  ...
  test("should navigate to filtered inventory when clicking category", async ({ page }) => {
    await page.goto("/");
...
test.describe("Accounting KPI Actionability", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });

  test("should navigate to invoices when clicking Accounts Receivable", async ({ page }) => {
  ...
  test("should navigate to bills when clicking Accounts Payable", async ({ page }) => {
    await page.goto("/accounting");
...
test.describe("Clients KPI Actionability", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });

  test("should filter table when clicking Clients with Debt KPI", async ({ page }) => {
...
test.describe("Inventory KPI Actionability", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStandardUser(page);
  });

  test("should filter by category when clicking category chart segment", async ({ page }) => {
    await page.goto("/inventory");
    
    // Find category chart or list and click a category
    const categoryItem = page.locator('text=Flower, text=Concentrates, text=Edibles').first();
