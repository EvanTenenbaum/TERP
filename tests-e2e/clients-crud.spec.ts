...
test.describe('Clients CRUD Operations', () => {
  ...
  test('should display clients table with data', async ({ page }) => {
    ...
    await page.waitForSelector('table, [role="table"], .data-table', { timeout: 5000 });
  ...
  test('should create a new client', async ({ page }) => {
    ...
    await page.fill('input[name="name"], input[placeholder*="name" i]', 'Test Client E2E');
    await page.fill('input[name="email"], input[type="email"]', 'testclient@example.com');
  ...
  test('should delete a client', async ({ page }) => {
    ...
    // Find delete button (could be in actions menu)
    ...
    if (await deleteButton.isVisible()) {
      ...
      // Success message
      await expect(page.locator('.toast, [role="alert"]').filter({ hasText: /success|deleted/i })).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should paginate through clients', async ({ page }) => {
    ...
    if (await nextButton.isVisible() && await nextButton.isEnabled()) {
      ...
      // Table should update with new page
      await page.waitForTimeout(500);
      await expect(page.locator('tbody tr, [role="row"]').first()).toBeVisible();
    }
  });
