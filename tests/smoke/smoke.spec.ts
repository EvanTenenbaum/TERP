...
test.setTimeout(IS_PRODUCTION ? 60000 : 30000);
...
test.describe("Smoke Tests - Critical Paths", () => {
  ...
  test("Dashboard loads successfully", async ({ page }) => {
    ...
    await expect(page.getByText(/error/i).first()).not.toBeVisible({ timeout: 2000 }).catch(() => {
      // Some "error" text might be in expected UI, so we just log
      console.log("Note: 'error' text found on page - verify if expected");
    });
  ...
  test("Products page shows products (QA-049)", async ({ page }) => {
    ...
    await page.waitForLoadState("networkidle", { timeout: 20000 }).catch(() => {
      console.log("Note: Network not fully idle, continuing...");
    });

    // Wait for skeleton to disappear
    const skeleton = page.locator('[data-testid="products-skeleton"]');
    await expect(skeleton).not.toBeVisible({ timeout: 15000 }).catch(() => {
      // Skeleton might not exist if already loaded
    });

    // Look for products table or grid
    const productsTable = page.locator('table').or(
    ...
    console.log(`✓ Products page shows ${rowCount} products`);
  });
  ...
  test("Samples page shows samples (QA-050)", async ({ page }) => {
    ...
    const samplesContent = page.locator('[class*="sample"]').or(
      page.locator('table')
    ).or(
    ...
    const tabsOrRows = page.locator('[role="tab"]').or(
    ...
    console.log(`✓ Samples page loaded with ${count} tabs/rows`);
  });
  ...
  test("Order Creator page loads (BUG-040 prerequisite)", async ({ page }) => {
    ...
    await expect(page.getByText(/404/i).or(page.getByText(/page not found/i))).not.toBeVisible({ timeout: 2000 }).catch(() => {});
    ...
    console.log("✓ Order Creator page loads correctly");
  });

  test("Inventory page and batch navigation (BUG-041 prerequisite)", async ({ page }) => {
    ...
    const inventoryContent = page.locator('table').or(
      page.locator('[class*="inventory"]')
    ).or(
      page.locator('[role="grid"]')
    );
    ...
    if (rowCount > 0) {
      console.log(`✓ Inventory page shows ${rowCount} batches`);
    } else {
      console.log("✓ Inventory page loaded (no batches in current view)");
    }
  });

  test("Global Search is accessible (BUG-042 prerequisite)", async ({ page }) => {
    await page.goto(`${BASE_URL}/`);
    ...
    const searchTrigger = page.locator('[class*="search"]').or(
    ...
    const hasSearchElement = await searchTrigger.first().isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (hasSearchElement) {
      ...
      const searchDialog = page.locator('[role="dialog"]').or(
        page.locator('[class*="command"]')
      );

      const hasDialog = await searchDialog.first().isVisible({ timeout: 2000 }).catch(() => false);
      if (hasDialog) {
        console.log("✓ Global search accessible via Cmd+K");
        await page.keyboard.press("Escape");
      } else {
        console.log("Note: Global search element not immediately visible - may require interaction");
      }
    }
  });
  ...
  test("All main navigation links work", async ({ page }) => {
    ...
    for (const link of navLinks) {
      ...
      const has404 = await page.getByText(/404|page not found/i).first().isVisible({ timeout: 1000 }).catch(() => false);
      const hasCriticalError = await page.getByText(/something went wrong|fatal error|application error/i).first().isVisible({ timeout: 1000 }).catch(() => false);

      if (has404) {
        results.push({ name: link.name, status: "404" });
        console.log(`✗ ${link.name} (${link.path}) - 404 Not Found`);
      } else if (hasCriticalError) {
        results.push({ name: link.name, status: "error" });
        console.log(`✗ ${link.name} (${link.path}) - Critical Error`);
      } else {
  ...
  test("API health endpoint responds", async ({ request }) => {
    ...
    if (healthOk) {
      console.log(`✓ Health check passed: ${responseInfo}`);
    } else {
      console.log("Note: No standard health endpoint found - application may still be healthy");
    }
...
test.describe("Smoke Tests - Error Scenarios", () => {
  test("No JavaScript errors on dashboard load", async ({ page }) => {
    ...
    if (criticalErrors.length > 0) {
      ...
      console.log("✓ No critical JavaScript errors on dashboard");
    }
  ...
  test("No console errors on products page", async ({ page }) => {
    ...
    if (criticalErrors.length > 0) {
      console.log("JavaScript errors found on Products page:");
      criticalErrors.forEach(e => console.log(`  - ${e}`));
    } else {
      console.log("✓ No critical JavaScript errors on Products page");
    }
