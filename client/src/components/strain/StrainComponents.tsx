...
export function RelatedProducts({
  ...
  const { alternatives, parent, isLoading } = useStrainSuggestions(
    strainId,
    currentProductId
  );
...
export function OutOfStockAlert({
  ...
  const { isOutOfStock, hasAlternatives, alternatives, parent } =
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {(preferences as unknown as StrainPreference[])
          .slice(0, 5)
          .map(pref => (
  ...
  return (
    <div className="flex items-center gap-2">
      {family.parent?.category && (
        <StrainCategoryBadge category={family.parent.category} />
      )}
