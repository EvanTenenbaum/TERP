...
export const ClientProfitMarginLeaderboard = memo(function ClientProfitMarginLeaderboard() {
  ...
  const { data: response, isLoading, error } = trpc.dashboard.getClientProfitMargin.useQuery(
    { timePeriod },
      ...
      <CardContent>
        {isLoading ? (
          ...
          <EmptyState
            variant="generic"
            size="sm"
            title="Unable to load profit margin data"
          ...
          <EmptyState
            variant="analytics"
            size="sm"
            title="No profit margin data"
            description="Profit margin data will appear once sales are recorded"
          />
        )}
      </CardContent>
    </Card>
