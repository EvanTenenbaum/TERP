...
export function InspectorPanel({
  ...
  return (
    <InspectorPanelContext.Provider value={contextValue}>
      {/* Backdrop for mobile — pointer-events-none unless closeOnClickOutside to avoid
          blocking interaction with action buttons behind the panel (BUG-050) */}
      {isMobile && (
        <div
          className={cn(
            "fixed inset-0 bg-black/20 z-40 md:hidden",
            !closeOnClickOutside && "pointer-events-none"
          )}
      ...
      >
        {/* Header */}
        {/* BUG-040: Single close affordance — only the X button in the right header actions */}
