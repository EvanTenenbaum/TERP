...
export function InventoryManagementSurface() {
      ...
      Array.from(
        new Map(
          rows
            ...
            .map(row => [row.vendorName, { id: row.vendorName, name: row.vendorName }])
        ).values()
      ),
    [rows]
  );
      ...
      Array.from(
        new Map(
          rows
            ...
            .map(row => [row.brandName, { id: row.brandName, name: row.brandName }])
        ).values()
      ),
    [rows]
  );
  const gradeOptions = useMemo(
    () =>
      Array.from(new Set(rows.map(row => row.grade).filter(Boolean) as string[])).sort(
  ...
  const handleInspectorReviewAdjustment = useCallback(() => {
    ...
    if (nextValue === selectedRow.onHandQty) {
      toast.error("Change the on-hand quantity before opening the adjustment review.");
      return;
    }
    setAdjustDrawerState({
      isOpen: true,
