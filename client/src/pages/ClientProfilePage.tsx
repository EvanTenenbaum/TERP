...
export default function ClientProfilePage() {
  ...
  // BUG-104 fix: Show specific error message if query failed
  if (clientError) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center space-y-4">
          <AlertCircle className="h-12 w-12 text-destructive mx-auto mb-4" />
          <div className="text-lg font-medium">Error loading client</div>
          <p className="text-sm text-muted-foreground max-w-md">
            {clientError.message ||
              "Failed to load client data. Please try again."}
          </p>
          <Button onClick={() => refetchClient()}>Retry</Button>
        </div>
      </div>
    );
  }

  // BUG-104 fix: Only show "not found" if query succeeded but returned null
  if (!client) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Client with ID {clientId} does not exist or you don't have
