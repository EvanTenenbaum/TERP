...
export default function BankAccounts() {
  // Fetch bank accounts
  const { data: accounts, isLoading } = trpc.accounting.bankAccounts.list.useQuery({});
  ...
  const activeAccounts = accountList.filter((a: any) => a.isActive).length;
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(totalCash || 0)}</div>
          </CardContent>
        </Card>
      </div>
