  ...
  if (need.strainId && candidate.strainId) {
    ...
    if (need.strainId === candidate.strainId) {
      ...
      try {
        ...
        logger.error("Error checking strain family:", error);
      }
    }
...
async function calculateBatchSellingPrice(
  ...
  try {
    ...
    logger.error("Error calculating batch selling price:", error);
    return null;
  }
}
...
export async function getAllActiveNeedsWithMatches(): Promise<MatchResult[]> {
  ...
  try {
    ...
    logger.error("Error getting all active needs with matches:", error);
    throw new Error(
