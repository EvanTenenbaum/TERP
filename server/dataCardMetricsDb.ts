...
import { eq, count, sum, sql, and, gte, lte, desc, isNull } from "drizzle-orm";
import {
  batches,
  products,
  orders,
...
async function calculateInventoryMetrics(
  ...
  const [aggregates] = await db
    .select({
      totalValue: sum(
        sql`CAST(${batches.onHandQty} AS DECIMAL(15,2)) * CAST(${batches.unitCogs} AS DECIMAL(15,2))`
      ),
      totalUnits: sum(batches.onHandQty),
      avgCogs: sql<number>`AVG(CAST(${batches.unitCogs} AS DECIMAL(15,2)))`,
      awaitingIntake: count(
        sql`CASE WHEN ${batches.batchStatus} = 'AWAITING_INTAKE' THEN 1 END`
      ),
      quarantined: count(
        sql`CASE WHEN ${batches.batchStatus} = 'QUARANTINED' THEN 1 END`
      ),
      onHold: count(
        sql`CASE WHEN ${batches.batchStatus} = 'ON_HOLD' THEN 1 END`
      ),
      live: count(sql`CASE WHEN ${batches.batchStatus} = 'LIVE' THEN 1 END`),
      lowStock: count(
        sql`CASE WHEN (CAST(${batches.onHandQty} AS DECIMAL(15,2)) - CAST(${batches.reservedQty} AS DECIMAL(15,2)) - CAST(${batches.quarantineQty} AS DECIMAL(15,2)) - CAST(${batches.holdQty} AS DECIMAL(15,2))) <= 100 THEN 1 END`
      ),
      totalBatches: count(),
    })
    .from(batches);
...
async function calculateOrdersMetrics(
  ...
  const [aggregates] = await db
    .select({
      ...
      shipped: count(
        sql`CASE WHEN ${orders.fulfillmentStatus} = 'SHIPPED' THEN 1 END`
      ),
      delivered: count(
        sql`CASE WHEN ${orders.fulfillmentStatus} = 'DELIVERED' THEN 1 END`
      ),
  ...
  if (metricIds.includes("orders_this_week")) {
    ...
    const [result] = await db
      ...
      .from(orders)
      .where(
        and(eq(orders.orderType, "SALE"), gte(orders.createdAt, startOfWeek))
      );
...
async function calculateAccountingMetrics(
  ...
  if (metricIds.includes("accounting_revenue_mtd")) {
    ...
    const [result] = await db
      ...
      .from(invoices)
      .where(
        and(gte(invoices.createdAt, startOfMonth), eq(invoices.status, "PAID"))
      );
...
async function calculateVendorSupplyMetrics(
  ...
  const [aggregates] = await db
    .select({
      total: count(),
      available: count(
        sql`CASE WHEN ${vendorSupply.status} = 'AVAILABLE' THEN 1 END`
      ),
      reserved: count(
        sql`CASE WHEN ${vendorSupply.status} = 'RESERVED' THEN 1 END`
      ),
      purchased: count(
        sql`CASE WHEN ${vendorSupply.status} = 'PURCHASED' THEN 1 END`
      ),
      totalValue: sum(
        sql`CAST(${vendorSupply.quantityAvailable} AS DECIMAL(15,2)) * CAST(${vendorSupply.unitPrice} AS DECIMAL(15,2))`
      ),
    })
    .from(vendorSupply);
  ...
  if (metricIds.includes("vendor_by_vendor")) {
    ...
    // Get supplier name via canonical clients table
    // vendorSupply.vendorId is the legacy vendor ID; resolve via supplierProfiles
    let vendorName = "N/A";
    if (topVendor?.vendorId) {
      const supplierResult = await db
        ...
        .where(
          and(
            eq(supplierProfiles.legacyVendorId, topVendor.vendorId),
            eq(clients.isSeller, true),
            isNull(clients.deletedAt)
          )
        )
        .limit(1);
      vendorName = supplierResult[0]?.name || "N/A";
    }
...
async function calculateClientsMetrics(
  ...
  const [aggregates] = await db
    .select({
      ...
      withDebt: count(
        sql`CASE WHEN CAST(${clients.totalOwed} AS DECIMAL(15,2)) > 0 THEN 1 END`
      ),
