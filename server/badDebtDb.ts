...
export async function writeOffBadDebt(
  ...
  try {
    ...
    const originalTransaction = await transactionsDb.getTransactionById(transactionId);
    ...
    if (writeOffNum > originalAmount) {
      throw new Error("Write-off amount cannot exceed original transaction amount");
    }
...
export async function reverseBadDebtWriteOff(
  ...
  try {
    ...
    const writeOffTransaction = await transactionsDb.getTransactionById(writeOffTransactionId);
    ...
    if (!originalTransactionId) {
      throw new Error("Original transaction ID not found in write-off metadata");
    }
    
    const originalTransaction = await transactionsDb.getTransactionById(originalTransactionId);
    if (!originalTransaction) {
 ...
 */
export async function getBadDebtAgingReport(daysThreshold: number = 90): Promise<Array<{
  clientId: number;
  clientName: string;
  totalOverdue: number;
  ...
  try {
    ...
    for (const transaction of overdueTransactions) {
      ...
      const daysOverdue = Math.floor((now.getTime() - transactionDate.getTime()) / (24 * 60 * 60 * 1000));
