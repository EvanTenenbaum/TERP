...
export async function getInventoryWithPricing(
  ...
  if (!db) {
    logger.error({ clientId }, "BUG-040: Database not available for inventory fetch");
  ...
  try {
    ...
    logger.info({ clientId, limit, offset }, "BUG-040: Fetching inventory batches");
    ...
    logger.info({ clientId, batchCount: inventoryBatches.length }, "BUG-040: Batches fetched");
    ...
    let clientRules: Awaited<ReturnType<typeof pricingEngine.getClientPricingRules>> = [];
    try {
      clientRules = await pricingEngine.getClientPricingRules(clientId);
      logger.info({ clientId, ruleCount: clientRules.length }, "BUG-040: Pricing rules loaded");
    } catch (ruleError) {
    ...
    try {
      ...
      logger.error({ error: pricingError, clientId }, "BUG-040: Pricing engine error, using fallback pricing");
    ...
    logger.error({ error: errorMsg, clientId }, "BUG-040: Error fetching inventory");

    // BUG-040 FIX: Provide specific error message based on error type
    if (errorMsg.includes("timeout") || errorMsg.includes("ETIMEDOUT")) {
      throw new Error("Inventory loading timed out. The server may be under heavy load. Please try again.");
    }
