  ...
  try {
    ...
    for (const item of saleData.lineItems) {
      ...
      result.lineItems.push({
        ...
        adjustmentAmount:
          adjustedCost !== unitCost
            ? (adjustedCost - unitCost) * quantity
            : undefined,
      });
    ...
    throw new Error(
      `Failed to calculate COGS: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
 ...
 */
function applyClientCOGSAdjustment(
  baseCost: number,
  client: Client | undefined
): number {
  if (
    !client ||
    !client.cogsAdjustmentType ||
...
export async function calculateWeightedAverageCOGS(
  ...
  try {
    ...
    throw new Error(
      `Failed to calculate weighted average COGS: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
...
export async function calculateTotalInventoryValue(): Promise<number> {
  ...
  try {
    ...
    throw new Error(
      `Failed to calculate total inventory value: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
...
export async function getCOGSBreakdownByProduct(productId: number): Promise<{
  ...
  try {
    ...
    throw new Error(
      `Failed to get COGS breakdown: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
