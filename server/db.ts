...
export async function getDb(): Promise<MySql2Database<typeof schema>> {
  if (!_db) {
    try {
      ...
      throw new Error(
        "Database connection failed - cannot start server without database"
      );
    }
  }
  return _db;
...
function initSyncDb(): MySql2Database<typeof schema> {
  if (!_syncDb) {
    try {
      ...
      logger.warn({
        msg: "Database not available - operations will fail",
        error,
      });
      ...
      return new Proxy({} as MySql2Database<typeof schema>, {
        get(_target, prop) {
          ...
          throw new Error(
            `Database not available - cannot perform operation: ${String(prop)}`
          );
        },
      });
    }
...
/**
 * Return the total number of user records. Used by /api/auth/create-first-user
 * to block the endpoint once at least one account exists.
 */
