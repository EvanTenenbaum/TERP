    ...
    .input(
      z.object({
        ...
        eventTypes: z
          ...
          .optional(),
        statuses: z
          .array(z.enum(["SCHEDULED", "IN_PROGRESS", "COMPLETED", "CANCELLED"]))
          .optional(),
        priorities: z
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        const eventConditions: Array<
          ReturnType<
            typeof gte | typeof lte | typeof isNull | typeof inArray | typeof eq
          >
        > = [
        ...
        if (input.eventTypes && input.eventTypes.length > 0) {
          eventConditions.push(
            inArray(calendarEvents.eventType, input.eventTypes)
          );
        }
        ...
        if (input.priorities && input.priorities.length > 0) {
          eventConditions.push(
            inArray(calendarEvents.priority, input.priorities)
          );
        }
        if (input.assignedTo) {
        ...
        const instanceConditions: Array<ReturnType<typeof gte | typeof lte>> = [
          gte(
            calendarRecurrenceInstances.instanceDate,
  ...
  createEvent: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.participants && input.participants.length > 0) {
        usersToCheck.push(
          ...input.participants.filter(p => !usersToCheck.includes(p))
        );
      }
      ...
      if (usersToCheck.length > 0 && input.startTime && input.endTime) {
        ...
        if (conflictingEvents.length > 0) {
          const conflictInfo = conflictingEvents
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.version !== undefined) {
        ...
        await calendarDb.updateEvent(
          input.id,
          dbUpdates as Parameters<typeof calendarDb.updateEvent>[1]
        );
      }
