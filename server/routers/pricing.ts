...
export const pricingRouter = router({
  ...
  getClientContext: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return await orderPricingService.getClientPricingContext(
        input.clientId,
        userId
      );
    }),
  ...
  checkCredit: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await orderPricingService.checkClientCredit(
        input.clientId,
        input.orderTotal
      );
      return {
  ...
  updateVariableMarkupRule: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (data.thresholdMin !== undefined)
        updateData.thresholdMin = data.thresholdMin;
      if (data.thresholdMax !== undefined)
        updateData.thresholdMax = data.thresholdMax;
      if (data.adjustmentMode !== undefined)
        updateData.adjustmentMode = data.adjustmentMode;
  ...
  createRule: protectedProcedure
    ...
    .input(
      z.object({
        ...
        adjustmentValue: z
          .number()
          .min(-100, "Adjustment value cannot be less than -100%")
          .max(1000, "Adjustment value cannot exceed 1000%"),
