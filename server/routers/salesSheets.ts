...
const salesSheetItemSchema = z.object({
  ...
  unitCogs: z.number().min(0).finite().optional(),
  unitCogsMin: z.number().min(0).finite().nullable().optional(),
  unitCogsMax: z.number().min(0).finite().nullable().optional(),
  effectiveCogs: z.number().min(0).finite().optional(),
  effectiveCogsBasis: z.enum(["LOW", "MID", "HIGH", "MANUAL"]).optional(),
