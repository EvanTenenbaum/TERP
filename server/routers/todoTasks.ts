...
export const todoTasksRouter = router({
  ...
  getListTasks: requirePermission("todos:read")
    .input(
      z.object({
        listId: z.number(),
      })
  ...
  getMyTasks: requirePermission("todos:read").query(async ({ ctx }) => {
    if (!ctx.user) throw new Error("Unauthorized");
  ...
  complete: requirePermission("todos:read")
    .input(
      z.object({
        taskId: z.number(),
      })
  ...
  uncomplete: requirePermission("todos:read")
    .input(
      z.object({
        taskId: z.number(),
      })
  ...
  assign: requirePermission("todos:read")
    .input(
      z.object({
        taskId: z.number(),
  ...
  reorder: requirePermission("todos:read")
    .input(
      z.object({
        listId: z.number(),
  ...
  // Get overdue tasks
  getOverdue: requirePermission("todos:read").query(async ({ ctx }) => {
    if (!ctx.user) throw new Error("Unauthorized");
    return await todoTasksDb.getOverdueTasks();
  }),

  // Get tasks due soon
  getDueSoon: requirePermission("todos:read").query(async ({ ctx }) => {
    if (!ctx.user) throw new Error("Unauthorized");
    return await todoTasksDb.getTasksDueSoon();
  }),

  // Get task statistics for a list
  getListStats: requirePermission("todos:read")
    .input(
      z.object({
        listId: z.number(),
      })
