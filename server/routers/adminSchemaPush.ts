...
export const adminSchemaPushRouter = router({
  ...
  pushSchema: adminProcedure.mutation(async () => {
    ...
    const safeExecute = async (stepName: string, sqlStatement: ReturnType<typeof sql>) => {
      try {
        ...
        if (errorMessage.includes('Duplicate column') || 
          ...
          results.push({ step: stepName, status: 'error', message: errorMessage });
          return false;
        }
      }
    };
    ...
    try {
      ...
      // ============================================================================
      await safeExecute('0027_add_vendor_paymentTerms', sql`
      ...
      // ============================================================================
      await safeExecute('0028_create_vendorNotes', sql`
      ...
      // ============================================================================
      await safeExecute('0029_create_purchaseOrders', sql`
      ...
      await safeExecute('0029_create_purchaseOrderItems', sql`
      ...
      await safeExecute('0029_fk_purchaseOrders_vendor', sql`
        ALTER TABLE purchaseOrders 
        ADD CONSTRAINT fk_po_vendor FOREIGN KEY (vendorId) REFERENCES vendors(id) ON DELETE RESTRICT
      `);

      await safeExecute('0029_fk_purchaseOrders_createdBy', sql`
        ALTER TABLE purchaseOrders 
      ...
      // ============================================================================
      await safeExecute('0030_add_inventoryMovements_adjustmentReason', sql`
      ...
      // ============================================================================
      await safeExecute('0031_add_calendar_client_id', sql`
        ALTER TABLE calendar_events ADD COLUMN client_id INT NULL
      `);

      await safeExecute('0031_add_calendar_vendor_id', sql`
        ALTER TABLE calendar_events ADD COLUMN vendor_id INT NULL
      `);

      await safeExecute('0031_add_calendar_metadata', sql`
        ALTER TABLE calendar_events ADD COLUMN metadata JSON NULL
      `);

      await safeExecute('0031_idx_calendar_client_id', sql`
        CREATE INDEX idx_calendar_events_client_id ON calendar_events(client_id)
      `);

      await safeExecute('0031_idx_calendar_vendor_id', sql`
      ...
      // Risk: MEDIUM | Reversibility: HIGH
      // ============================================================================
      await safeExecute('0034_add_orders_intake_event_id', sql`
        ALTER TABLE orders ADD COLUMN intake_event_id INT NULL
      `);

      await safeExecute('0034_idx_orders_intake_event_id', sql`
      ...
      // Risk: MEDIUM | Reversibility: HIGH
      // ============================================================================
      await safeExecute('0035_add_batches_photo_session_event_id', sql`
        ALTER TABLE batches ADD COLUMN photo_session_event_id INT NULL
      `);

      await safeExecute('0035_idx_batches_photo_session_event_id', sql`
      ...
      // Risk: MEDIUM | Reversibility: HIGH
      // ============================================================================
      await safeExecute('0036_create_calendar_event_invitations', sql`
      ...
      // Risk: LOW | Reversibility: HIGH
      // ============================================================================
      await safeExecute('0038_idx_orders_client_id', sql`
        CREATE INDEX idx_orders_client_id ON orders(client_id)
      `);

      await safeExecute('0038_idx_invoices_order_id', sql`
        CREATE INDEX idx_invoices_order_id ON invoices(orderId)
      `);

      await safeExecute('0038_idx_payments_invoice_id', sql`
        CREATE INDEX idx_payments_invoice_id ON payments(invoiceId)
      `);

      await safeExecute('0038_idx_batches_product_id', sql`
      ...
      const softDeleteTables = [
        'users', 'clients', 'vendors', 'products', 'batches', 
        'orders', 'invoices', 'payments', 'strains'
      ];

      for (const table of softDeleteTables) {
        await safeExecute(`0039_add_${table}_deleted_at`, sql.raw(`
      ...
      // Risk: MEDIUM | Reversibility: HIGH
      // ============================================================================
      await safeExecute('0040_create_credit_visibility_settings', sql`
      ...
      await safeExecute('0040_insert_default_credit_visibility', sql`
      ...
      // Risk: MEDIUM | Reversibility: HIGH
      // ============================================================================
      await safeExecute('0041_create_leaderboard_weight_configs', sql`
      ...
      await safeExecute('0041_create_leaderboard_default_weights', sql`
      ...
      await safeExecute('0041_create_leaderboard_metric_cache', sql`
      ...
      await safeExecute('0041_create_leaderboard_rank_history', sql`
      ...
      await safeExecute('0041_create_dashboard_widget_configs', sql`
      ...
      // ============================================================================
      await safeExecute('0042_add_clients_credit_limit_updated_at', sql`
        ALTER TABLE clients ADD COLUMN credit_limit_updated_at TIMESTAMP NULL
      `);

      await safeExecute('0042_add_clients_creditLimitSource', sql`
        ALTER TABLE clients ADD COLUMN creditLimitSource ENUM('CALCULATED','MANUAL') DEFAULT 'CALCULATED'
      `);

      await safeExecute('0042_add_clients_credit_limit_override_reason', sql`
  ...
  verifySchema: adminProcedure.query(async () => {
    ...
    try {
      ...
      `);
      const tableNames = (allTables as unknown as Array<{ TABLE_NAME: string }>).map(row => row.TABLE_NAME);
      ...
      const criticalTables = {
        ...
        calendar_event_invitations: tableNames.includes('calendar_event_invitations'),
        credit_visibility_settings: tableNames.includes('credit_visibility_settings'),
        leaderboard_weight_configs: tableNames.includes('leaderboard_weight_configs'),
        leaderboard_default_weights: tableNames.includes('leaderboard_default_weights'),
        leaderboard_metric_cache: tableNames.includes('leaderboard_metric_cache'),
        leaderboard_rank_history: tableNames.includes('leaderboard_rank_history'),
        dashboard_widget_configs: tableNames.includes('dashboard_widget_configs')
      };
      ...
      `);
      const vendorsCols = (vendorsColumns as unknown as Array<{ COLUMN_NAME: string }>).map(row => row.COLUMN_NAME);
      ...
      `);
      const calendarCols = (calendarColumns as unknown as Array<{ COLUMN_NAME: string }>).map(row => row.COLUMN_NAME);
      ...
      if (criticalTables.purchaseOrders) {
        ...
        `);
        const poCols = (poColumns as unknown as Array<{ COLUMN_NAME: string }>).map(row => row.COLUMN_NAME);
