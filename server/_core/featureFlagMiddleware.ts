...
export function requireModule(module: string) {
  return async ({ ctx, next }: { ctx: any; next: () => Promise<any> }) => {
    const isEnabled = await featureFlagService.isModuleEnabled(module, {
...
export function whenFeatureEnabled(
  flagKey: string,
  middleware: ({ ctx, next }: { ctx: any; next: () => Promise<any> }) => Promise<any>
) {
  return async ({ ctx, next }: { ctx: any; next: () => Promise<any> }) => {
    const isEnabled = await featureFlagService.isEnabled(flagKey, {
      userOpenId: ctx.user?.openId,
...
export function requireFlags(flags: string[], mode: "all" | "any" = "all") {
  return async ({ ctx, next }: { ctx: any; next: () => Promise<any> }) => {
    const context = ctx.user?.openId ? { userOpenId: ctx.user.openId } : undefined;
