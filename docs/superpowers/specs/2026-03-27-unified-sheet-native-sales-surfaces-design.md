...
- **Payment Terms**: Select dropdown (Net 15 / Net 30 / Net 45 / Net 60 / COD / Prepaid) + "★ Save default" action to persist to customer profile. Auto-populated from customer default on client selection. *Note: Payment Terms in the order creation UI is new — requires schema investigation during implementation for whether the field already exists on the order/client tables.*
