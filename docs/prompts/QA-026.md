...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-026"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-026-$(openssl rand -hex 4)"
...
echo "- QA-026: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-026"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-026: [Description of change]"
   ```
...
```bash
cat > "docs/QA-026-COMPLETION-REPORT.md" << 'EOF'
# QA-026 Completion Report
...
1. **Update roadmap status:**
   ...
   **Resolution:** [Brief summary of what was done]
   See docs/QA-026-COMPLETION-REPORT.md for details.
   ```
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-026: Conduct Performance Testing
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-026-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-026-fix"
   git push origin qa-026-fix:main
   ```
