...
**Estimated Time:** 22 hours (revised from 16h after QA)
**Risk Level:** HIGH — Production code affecting all data-displaying features
**Last Updated:** 2025-12-22 (Red Hat QA Level 3 corrections applied)

---

## 🚨 CRITICAL PRE-EXECUTION FINDINGS (READ FIRST)
...
### Finding 2: Total Procedures Requiring Work is 27

**⚠️ CORRECTED 2025-12-22 (Red Hat QA Level 3):** The original analysis incorrectly claimed strains.ts and inventory.ts "already use Contract B". They have **unmarked inline wrappers** with the same pattern as BUG-033 hotfixes.

| Router            | BUG-033 Marked | Unmarked Wrappers | Total  | Notes                                                                      |
| ----------------- | -------------- | ----------------- | ------ | -------------------------------------------------------------------------- |
| accounting.ts     | 8              | 0                 | 8      | Domain-keyed DB responses                                                  |
| strains.ts        | 0              | **3**             | 3      | list, search, fuzzySearch have inline wrappers                             |
| inventory.ts      | 0              | **5**             | 5      | suppliers, brands, views.list, profitability.top (NOT list - it's correct) |
| todoTasks.ts      | 2              | 0                 | 2      | getOverdue, getDueSoon (getListTasks/getMyTasks fixed in pre-work)         |
| todoLists.ts      | 2              | 0                 | 2      | getMyLists, getMembers                                                     |
| inbox.ts          | 2              | 0                 | 2      | getUnread, getByStatus                                                     |
| clients.ts        | 1              | 0                 | 1      | list                                                                       |
| orders.ts         | 1              | 0                 | 1      | getAll                                                                     |
| purchaseOrders.ts | 1              | 0                 | 1      | getAll                                                                     |
| samples.ts        | 1              | 0                 | 1      | getPending                                                                 |
| suppliers.ts      | 1              | 0                 | 1      | getAll                                                                     |
| **TOTAL**         | **19**         | **8**             | **27** |                                                                            |

**Search command to find ALL wrappers (not just BUG-033 marked):**

```bash
grep -r "nextCursor: null" server/routers --include="*.ts"
```
...
### Finding 5: Frontend Already Has Defensive Patterns (No Changes Needed)

**⚠️ IMPORTANT:** The frontend already handles multiple response shapes with defensive patterns:
...
**Files using this pattern:** 20+ components including:

- `TodoListsPage.tsx`, `Orders.tsx`, `Quotes.tsx`, `ClientsListPage.tsx`
- `PurchaseOrdersPage.tsx`, `OrderCreatorPage.tsx`, `TodoListDetailPage.tsx`
- `AppHeader.tsx`, `InboxPanel.tsx`, `InboxWidget.tsx`, `CommentWidget.tsx`

**Implication:** As long as responses include the `items` key, frontend will work without changes.

### Finding 6: Execution Strategy Recommendation

**⚠️ CRITICAL:** Do NOT refactor all DB files first, then all routers. This risks double-wrapping.

**Recommended approach:** For each endpoint, refactor DB AND router together, then test:

1. Refactor `strainsDb.getAllStrains` → Test → Refactor `strains.list` router → Test
2. Repeat for each endpoint

This ensures no intermediate state where DB returns structured data but router still wraps it.
...
3. **NEVER assume a change is safe — verify with tests.**
4. **NEVER skip a QA gate — they exist to prevent cascading failures.**
...
1. `nextCursor: null` — No true cursor-based pagination
2. `hasMore` is a heuristic — Incorrect at pagination boundaries
3. `total` is unknown — Frontend pagination UI may break
4. Wrappers are in the router layer — Should be in the DB layer
...
📋 Files Modified Since Last Gate:
- [List files]
...
### ⚠️ CORRECTED INVENTORY (Post Red Hat QA)
...
| `server/routers/suppliers.ts`      | getAll                                                                                                                                 | 1            | DB returns raw array            |

**Subtotal: 9 router files, 19 procedures with BUG-033 hotfixes**

### Router Files WITH UNMARKED Inline Wrappers (8 procedures):

**⚠️ CRITICAL: These have the SAME pattern as BUG-033 hotfixes but NO comment marker. They MUST be included in BUG-034 scope.**

| File                          | Procedures                                                           | Count | Notes                                                         |
| ----------------------------- | -------------------------------------------------------------------- | ----- | ------------------------------------------------------------- |
| `server/routers/strains.ts`   | list, search, fuzzySearch                                            | 3     | Returns `{ items: result, nextCursor: null, hasMore: false }` |
| `server/routers/inventory.ts` | suppliers (2 paths), brands (2 paths), views.list, profitability.top | 5     | Same pattern, no BUG-033 marker                               |

**Subtotal: 2 router files, 8 procedures with unmarked wrappers**

### Router Files CORRECTLY Using Cursor Pagination (NO changes needed):

| File                          | Procedures | Status     | Notes                                      |
| ----------------------------- | ---------- | ---------- | ------------------------------------------ |
| `server/routers/inventory.ts` | list       | ✅ CORRECT | Uses cursor-based pagination from DB layer |

**This 1 procedure is already correct and does NOT need BUG-034 changes.**

### Router Files ALREADY Fixed (Phase 1 Pre-work):

| File                          | Procedures               | Status   | Notes                                    |
| ----------------------------- | ------------------------ | -------- | ---------------------------------------- |
| `server/routers/todoTasks.ts` | getListTasks, getMyTasks | ✅ FIXED | Now extracts from structured DB response |

**These 2 procedures were fixed during BUG-034 pre-work (2025-12-22).**

### TOTAL PROCEDURES REQUIRING BUG-034 WORK: 27

| Category                 | Count  |
| ------------------------ | ------ |
| BUG-033 marked hotfixes  | 19     |
| Unmarked inline wrappers | 8      |
| **TOTAL**                | **27** |
...
| --------------------------------------- | ------------------------------------------ | ------------------------------------- |
| `todoTasksDb.getListTasks`              | `{ items, total, limit, offset, hasMore }` | ✅ Already structured - extract items |
| `todoTasksDb.getUserAssignedTasks`      | `{ items, total, limit, offset, hasMore }` | ✅ Already structured - extract items |
...
| `vendorsDb.getAllVendors`               | `Supplier[]`                               | Add count query, wrap                 |
...
- [ ] **Verify BUG-033 hotfix count:** Run `grep -r "HOTFIX (BUG-033)" server/routers --include="*.ts" | wc -l` — Should be 19
- [ ] **Verify ALL wrappers (including unmarked):** Run `grep -r "nextCursor: null" server/routers --include="*.ts" | wc -l` — Should be ~27
- [ ] **Verify strains.ts has unmarked wrappers:** Check for `{ items: result, nextCursor: null, hasMore: false }` pattern (3 procedures)
- [ ] **Verify inventory.ts has unmarked wrappers:** Check suppliers, brands, views.list, profitability.top (5 procedures)
- [ ] **Verify inventory.ts list is CORRECT:** Should pass through DB response directly (no wrapper)
...
- [ ] **Run TypeScript check:** `pnpm check` — Should pass
- [ ] **Run tests:** `pnpm test --run` — Record baseline
...
**🔒 QA GATE 0: BASELINE VERIFICATION**
...
**🔒 QA GATE 1: PAGINATION INFRASTRUCTURE**

---

### ═══════════════════════════════════════════════════════════════

### PHASE 2: REFACTOR DB LAYER — ONE FILE AT A TIME (8 hours)

### ═══════════════════════════════════════════════════════════════

**Objective:** Refactor each DB file to return `PaginatedResult<T>` directly.

**⚠️ CRITICAL: Process ONE file at a time. Run tests after EACH file.**
...
export async function getAllOrders(input: {
  ...
  const orders = await query.orderBy(asc(ordersTable.id)).limit(limit + 1);
...
1. `server/strainsDb.ts` — Low complexity, isolated module
2. `server/samplesDb.ts` — Low complexity, isolated module
3. `server/inboxDb.ts` — Low complexity, isolated module
4. `server/todoListsDb.ts` — Low complexity, isolated module
5. `server/todoTasksDb.ts` — Medium complexity
6. `server/clientsDb.ts` — Medium complexity
7. `server/vendorsDb.ts` (if exists, or functions in inventoryDb) — Medium complexity
8. `server/purchaseOrdersDb.ts` — Medium complexity
9. `server/ordersDb.ts` — High complexity, critical path
10. `server/accountingDb.ts` — High complexity
11. `server/arApDb.ts` — High complexity
12. `server/cashExpensesDb.ts` — High complexity
13. `server/inventoryDb.ts` — Highest complexity, most critical
...
**🔒 QA GATE 2A: After strainsDb, samplesDb, inboxDb**
**🔒 QA GATE 2B: After todoListsDb, todoTasksDb, clientsDb**
**🔒 QA GATE 2C: After vendorsDb, purchaseOrdersDb, ordersDb**
**🔒 QA GATE 2D: After accountingDb, arApDb, cashExpensesDb**
**🔒 QA GATE 2E: After inventoryDb (CRITICAL)**

---

### ═══════════════════════════════════════════════════════════════

### PHASE 3: UPDATE ROUTER LAYER — ONE FILE AT A TIME (4 hours)
...
**🔒 QA GATE 3A: After strains, samples, inbox routers**
**🔒 QA GATE 3B: After todoLists, todoTasks, clients routers**
**🔒 QA GATE 3C: After suppliers, purchaseOrders, orders routers**
**🔒 QA GATE 3D: After accounting router**
**🔒 QA GATE 3E: After inventory router (CRITICAL)**
...
2. **Verify each major page loads correctly:**
   - [ ] Dashboard — All widgets display data
   - [ ] Inventory — List loads, pagination works
   - [ ] Orders — List loads, pagination works
   - [ ] Clients — List loads, pagination works
   - [ ] Accounting — All sub-pages load
   - [ ] Purchase Orders — List loads
   - [ ] Todo Lists — Lists and tasks load
...
**🔒 QA GATE 4: FRONTEND VERIFICATION**
...
4. **Search for any remaining BUG-033 hotfix comments:**
   ...
   - If any remain, they were missed — go back and fix them
...
**🔒 QA GATE 5: FINAL VERIFICATION (CRITICAL)**
...
**🔒 QA GATE 6: POST-DEPLOYMENT VERIFICATION**
...
- [ ] All 8 unmarked inline wrappers removed (strains.ts: 3, inventory.ts: 5)
- [ ] **Total: 27 procedures standardized**
