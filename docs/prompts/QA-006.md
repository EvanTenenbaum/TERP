...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-006"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-006-$(openssl rand -hex 4)"
...
echo "- QA-006: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-006"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-006: [Description of change]"
   ```
...
```bash
cat > "docs/QA-006-COMPLETION-REPORT.md" << 'EOF'
# QA-006 Completion Report

**Task:** Fix Dashboard - Suppliers Button 404
...
1. **Update roadmap status:**
   ...
   **Resolution:** [Brief summary of what was done]
   See docs/QA-006-COMPLETION-REPORT.md for details.
   ```
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-006: Fix Dashboard - Suppliers Button 404
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-006-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-006-fix"
   git push origin qa-006-fix:main
   ```
