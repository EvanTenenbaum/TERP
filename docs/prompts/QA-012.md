...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-012"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-012-$(openssl rand -hex 4)"
...
echo "- QA-012: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-012"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-012: [Description of change]"
   ```
...
```bash
cat > "docs/QA-012-COMPLETION-REPORT.md" << 'EOF'
...
1. **Update roadmap status:**
   ...
   **Change from:**
   ...
   ```markdown
   **Priority:** P1 | **Status:** Not Started | **Effort:** 8-12 hours
   ```

   **Change to:**

   ```markdown
   **Priority:** P1 | **Status:** ✅ Complete (2025-11-14) | **Effort:** 8-12 hours
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-012: Fix Global Search Functionality
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-012-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-012-fix"
   git push origin qa-012-fix:main
   ```
