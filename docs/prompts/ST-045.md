...
You have:
• Full access to the most recent ERP web app codebase and all embedded documentation (business rules, specs, comments, markdown, etc.).
• Direct access to the ERP database (read/write) with the ability to:
...
(2) the database as a verification/expansion source of truth for: - actual lifecycle/status values present - schema constraints that create edge cases - RBAC role→permission mappings actually deployed - data integrity hazards (nulls, orphan rows, impossible states)

**Success Criteria:**

- Complete User Flow Matrix covering all entities and flows
- Structured Flow Guide organized by Domain → Entity → Role → Task
...
1. Enumerate:
   • Domains (Inventory, Sales, Purchasing, CRM, Accounting, Reporting, Admin, Settings, Integrations, VIP Portal, Workflow/Todos/Calendar, etc.)
   • Entities per domain (Product, Batch/Lot, Customer/Client, Supplier, Order, Invoice, Payment, Purchase Order, Shipment, Adjustment, Return, Note, Tag, User, Role, Permission, etc.)
   • Routes/pages: extract the full client route registry and the nav/menu definitions.
   • API surface: extract every API handler/procedure and its auth/permission requirements.
   • Workflow states: identify status fields/enums and any state-machine-like guards.

### B) DB scan:

2. Introspect schema and capture:
   • Full table list + primary keys + foreign keys
   • Enums or "status" fields (either DB enums or constrained text fields) and constraints
   • Unique constraints and NOT NULL constraints that drive validation edge cases
   • Cascade rules and orphan-risk areas

3. RBAC truth from DB:
   • Extract every role, permission, and role_permissions mapping from DB
   • Extract any user permission overrides
   • Compare DB RBAC to code RBAC seeds (if present) and report mismatches:
...
• LINK/ASSOCIATE
• STATE TRANSITIONS
• ACTIONS/OPERATIONS (export, print, reconcile, sync, approve, post, etc.)
• REPORTING/ANALYTICS
• ADMIN/SETTINGS

For each flow:
• Identify involved routes/pages/components
• Identify involved backend handlers/services
• Identify DB tables touched (directly or via service layer)
...
• Determine exact role/permission requirements from backend authorization.
• Cross-check with DB role_permissions to list which deployed roles can actually execute.

### 2) STATE

• For stateful entities, enumerate allowed actions per state using:
• code guards/branching (service + router logic)
• DB observed values (SELECT DISTINCT status FROM <table>)
...
• tax/currency logic (if present)
• approval requirements
• supplier/client active flags
• integration failure/retry paths
• Validate risk areas using DB constraints + real data patterns (nulls reminded by NOT NULL fails, uniqueness, FK violations, etc.)
...
• page buttons/links → other routes/actions
• If a backend flow has no route and no client reference, classify as "API-only".

### 5) HAPPY VS SAD PATHS

• Enumerate success + error branches:
• validation errors
• permission errors
• not found / stale state
• conflict/constraint failures
• integration errors (if any)
• Where possible, tie sad paths to concrete DB constraints or enum/state conflicts.
...
• Client wiring exists (or does not) and classify:
...
- Dead/legacy: unreachable/unreferenced/obsolete guards or routes
  • Compare "permissions required by endpoint" vs "permissions present in DB" and flag mismatches.
...
• Flow Name
• Archetype
• Role(s) (DB-derived, not just seed)
• Permission(s) required
• State(s) (and how determined: code enum vs DB distinct values)
• Key Conditions / Variants
• UI Entry Path(s) (route + component + link/button source, if inferable)
• Preconditions
• Postconditions (state changes, side effects)
• Related Entities
• Related Reports/Dashboards
• DB Tables touched (and key fields)
• Implementation Status (Fully wired / Partially wired / API-only / Dead)
• Known Issues / Uncertainties
• Documentation Status
• QA Status (Code-Level)
...
Organize:
Domain → Entity → Role → Task (Flow)

For each flow row:
• Business purpose
• Preconditions
• Happy path steps (conceptual UI steps derived from routes/components)
• Backend path (handler/service functions)
• DB impact (tables/fields, state transitions)
• Key variants (role/state/conditions)
• Validations & error states
• Accounting/inventory/reporting impacts where applicable
• Implementation status + contradictions/mismatches
...
**Quality bar:**
• No guessing. If unclear, mark uncertainty and point to the exact code/DB evidence that's missing.
• Prefer concrete names: routes, components, procedures, service functions, tables, columns, permission strings.
• Explicitly call out RBAC mismatches, dead flows, and contradictions.
