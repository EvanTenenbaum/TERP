...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-002"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-002-$(openssl rand -hex 4)"
...
echo "- QA-002: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-002"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-002: [Description of change]"
   ```
...
```bash
cat > "docs/QA-002-COMPLETION-REPORT.md" << 'EOF'
# QA-002 Completion Report

**Task:** Fix 404 Error - Accounting Module
...
1. **Update roadmap status:**
   ...
   **Change from:**
   ...
   ```markdown
   **Priority:** P0 | **Status:** Not Started | **Effort:** 8-16 hours
   ```

   **Change to:**

   ```markdown
   **Priority:** P0 | **Status:** ✅ Complete (2025-11-14) | **Effort:** 8-16 hours

   **Resolution:** [Brief summary of what was done]
   See docs/QA-002-COMPLETION-REPORT.md for details.
   ```
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-002: Fix 404 Error - Accounting Module
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-002-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-002-fix"
   git push origin qa-002-fix:main
   ```
