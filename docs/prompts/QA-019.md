...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-019"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-019-$(openssl rand -hex 4)"
...
echo "- QA-019: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-019"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-019: [Description of change]"
   ```
...
```bash
cat > "docs/QA-019-COMPLETION-REPORT.md" << 'EOF'
...
1. **Update roadmap status:**
   ...
   **Change from:**
   ...
   ```markdown
   **Priority:** P2 | **Status:** Not Started | **Effort:** 2-4 hours
   ```

   **Change to:**

   ```markdown
   **Priority:** P2 | **Status:** ✅ Complete (2025-11-14) | **Effort:** 2-4 hours
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-019: Fix Credit Settings - Reset to Defaults Button
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-019-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-019-fix"
   git push origin qa-019-fix:main
   ```
