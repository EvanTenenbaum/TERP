...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-007"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-007-$(openssl rand -hex 4)"
...
echo "- QA-007: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-007"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-007: [Description of change]"
   ```
...
```bash
cat > "docs/QA-007-COMPLETION-REPORT.md" << 'EOF'
# QA-007 Completion Report

**Task:** Fix Dashboard - Purchase Orders Button 404
...
1. **Update roadmap status:**
   ...
   **Change from:**
   ...
   ```markdown
   **Priority:** P1 | **Status:** Not Started | **Effort:** 2-4 hours
   ```

   **Change to:**

   ```markdown
   **Priority:** P1 | **Status:** ✅ Complete (2025-11-14) | **Effort:** 2-4 hours

   **Resolution:** [Brief summary of what was done]
   See docs/QA-007-COMPLETION-REPORT.md for details.
   ```
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-007: Fix Dashboard - Purchase Orders Button 404
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-007-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-007-fix"
   git push origin qa-007-fix:main
   ```
