...
### Table: orders
- `client_id` → `clients.id` (MISSING INDEX)
- `vendor_id` → `suppliers.id` (MISSING INDEX)

### Table: inventory
- `product_id` → `products.id` (MISSING INDEX)
- `location_id` → `locations.id` (MISSING INDEX)
...
5. **Follow protocols** - See `docs/ROADMAP_AGENT_GUIDE.md`
