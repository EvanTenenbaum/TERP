...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-010"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-010-$(openssl rand -hex 4)"
...
echo "- QA-010: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-010"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-010: [Description of change]"
   ```
...
```bash
cat > "docs/QA-010-COMPLETION-REPORT.md" << 'EOF'
...
1. **Update roadmap status:**
   ...
   **Change from:**
   ...
   ```markdown
   **Priority:** P1 | **Status:** Not Started | **Effort:** 4-6 hours
   ```

   **Change to:**

   ```markdown
   **Priority:** P1 | **Status:** ✅ Complete (2025-11-14) | **Effort:** 4-6 hours
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-010: Fix Inventory - Export CSV Button
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-010-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-010-fix"
   git push origin qa-010-fix:main
   ```
