# QA-044: Apply Event Invitation Database Migration
...
1. Create session file: `docs/sessions/active/Session-$(date +%Y%m%d)-QA-044-$(openssl rand -hex 4).md`
...
3. If clear, add your session to the file:
   ```bash
   echo "- QA-044: Session-$(date +%Y%m%d)-QA-044-$(openssl rand -hex 4) ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
   ```
4. Commit and push **immediately**:
   ```bash
   git add docs/ACTIVE_SESSIONS.md
   git commit -m "Register session for QA-044"
...
git commit -m "Complete QA-044: Apply event invitation database migration"
