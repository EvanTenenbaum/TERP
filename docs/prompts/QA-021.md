...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-021"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-021-$(openssl rand -hex 4)"
...
echo "- QA-021: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-021"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-021: [Description of change]"
   ```
...
```bash
cat > "docs/QA-021-COMPLETION-REPORT.md" << 'EOF'
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-021: Test and Fix Pricing Rules - Create Rule Form
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-021-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-021-fix"
   git push origin qa-021-fix:main
   ```
