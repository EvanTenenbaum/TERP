...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-014"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-014-$(openssl rand -hex 4)"
...
echo "- QA-014: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-014"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-014: [Description of change]"
   ```
...
```bash
cat > "docs/QA-014-COMPLETION-REPORT.md" << 'EOF'
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-014: Fix Workflow Queue - History Button 404
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-014-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-014-fix"
   git push origin qa-014-fix:main
   ```
