...
2. **Read the roadmap:**
   ...
   ```bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "QA-024"
   ```
...
4. **Create session file:**
   ```bash
   SESSION_ID="Session-$(date +%Y%m%d)-QA-024-$(openssl rand -hex 4)"
...
echo "- QA-024: ${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
git add docs/ACTIVE_SESSIONS.md docs/sessions/active/${SESSION_ID}.md
git commit -m "Register session for QA-024"
...
4. **Commit frequently:**
   ```bash
   git add -A
   git commit -m "QA-024: [Description of change]"
   ```
...
```bash
cat > "docs/QA-024-COMPLETION-REPORT.md" << 'EOF'
# QA-024 Completion Report

**Task:** Test Settings - Form Submissions
...
1. **Update roadmap status:**
   ...
   **Change from:**
   ...
   ```markdown
   **Priority:** P3 | **Status:** Not Started | **Effort:** 6-8 hours
   ```

   **Change to:**

   ```markdown
   **Priority:** P3 | **Status:** ✅ Complete (2025-11-14) | **Effort:** 6-8 hours

   **Resolution:** [Brief summary of what was done]
   See docs/QA-024-COMPLETION-REPORT.md for details.
   ```
...
4. **Commit all changes:**
   ```bash
   git add -A
   git commit -m "Complete QA-024: Test Settings - Form Submissions
   ```
...
5. **Push directly to main (with merge conflict handling):**
   ...
   ```bash
   # Attempt to push
   git push origin qa-024-fix:main
   ```
   ...
   **If push fails (another agent pushed first):**
   ...
   git pull origin main
   # Resolve conflicts
   git commit -m "Merge main into qa-024-fix"
   git push origin qa-024-fix:main
   ```
