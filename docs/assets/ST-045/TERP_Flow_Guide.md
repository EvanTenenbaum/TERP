...
##### Pay Supplier (`accounting.payVendor`)

- **Purpose:** WS-002: Pay supplier (cash out)
...
- **Purpose:** Get comprehensive AP summary by supplier
...
##### Get Recent Suppliers (`accounting.getRecentVendors`)

- **Purpose:** Get recent suppliers for quick selection
...
##### Get Supplier Balance Breakdown (`audit.getVendorBalanceBreakdown`)

- **Purpose:** /\*\* Get supplier balance breakdown Shows all bills and payments
...
- **Purpose:** Part of Canonical Model Unification - replaces supplier profile functionality
...
- **Purpose:** /\*\* Set expected harvest date for a supplier
...
##### Get Supplier History (`vendorReminders.getVendorHistory`)

- **Purpose:** /\*\* Get supplier harvest history
...
- **Purpose:** /\*\* Expire old supplier supply items
...
- **Related UI routes:** /supplier-supply
...
- **Purpose:** /\*\* Mark a supplier supply item as purchased
...
- **Purpose:** /\*\* Mark a supplier supply item as reserved
...
- **Purpose:** /\*\* Create a new supplier supply item
...
- **Purpose:** /\*\* Delete a supplier supply item
...
- **Purpose:** /\*\* Update a supplier supply item
...
- **Purpose:** /\*\* Find potential buyers for a supplier supply item
...
- **Purpose:** /\*\* Get all supplier supply items with optional filters
...
- **Purpose:** /\*\* Get supplier supply with match counts
...
- **Purpose:** /\*\* Get available supplier supply items
...
- **Purpose:** /\*\* Get a supplier supply item by ID
...
- **Purpose:** /\*\* Create a new supplier (facade - creates client + supplier_profile) @deprecated Use clients.create with isSeller=true instead
...
- **Related UI routes:** /suppliers, /suppliers/:id

- **Implementation status:** API-only / Unreferenced in client

- **Source:** `server/routers/suppliers.ts`

##### Create Note (`suppliers.createNote`)

- **Purpose:** /\*\* Create a new note for a supplier Feature: MF-016 Supplier Notes & History
...
- **Purpose:** /\*\* Delete a supplier (facade - soft deletes client) @deprecated Use clients.delete instead
...
##### Delete Note (`suppliers.deleteNote`)

- **Purpose:** /\*\* Delete a supplier note Feature: MF-016 Supplier Notes & History
...
- **Purpose:** /\*\* Update an existing supplier (facade - updates client + supplier_profile) @deprecated Use clients.update instead
...
##### Update Note (`suppliers.updateNote`)

- **Purpose:** /\*\* Update a supplier note Feature: MF-016 Supplier Notes & History
...
##### Get All (`suppliers.getAll`)

- **Purpose:** /\*\* Get all suppliers (facade over getAllSuppliers) @deprecated Use clients.list with clientTypes=['seller'] instead
...
##### Get By Id (`suppliers.getById`)

- **Purpose:** /\*\* Get supplier by ID (facade - tries legacy supplier ID first, then client ID) @deprecated Use clients.getById instead
...
##### Get History (`suppliers.getHistory`)

- **Purpose:** /\*\* Get supplier history from audit logs Feature: MF-016 Supplier Notes & History
...
##### Get Notes (`suppliers.getNotes`)

- **Purpose:** /\*\* Get all notes for a supplier Feature: MF-016 Supplier Notes & History
...
- **Purpose:** /\*\* Search suppliers by name (facade over searchSuppliers) @deprecated Use clients.list with search and clientTypes=['seller'] instead
...
- **Purpose:** Get suppliers (for autocomplete)
...
##### Get Batches By Supplier (`inventory.getBatchesByVendor`)
...
- **Purpose:** Generate supplier receipt
...
##### Request Supplier Return (`samples.requestVendorReturn`)

- **Purpose:** Request a supplier return
...
##### Confirm Supplier Return (`samples.confirmVendorReturn`)

- **Purpose:** Confirm supplier received the sample
...
##### Ship To Supplier (`samples.shipToVendor`)

- **Purpose:** Ship sample to supplier
...
- **Purpose:** /\*_ Get counts of all seeded tables (using COUNT instead of SELECT _)
...
##### Find Matches For Supplier Supply (`matchingEnhanced.findMatchesForVendorSupply`)

- **Purpose:** /\*\* Find client needs that match a specific supplier supply
...
- **Purpose:** PO Intake Router API endpoints for intake purchase orders and updating inventory
...
- **Purpose:** Enhanced goods intake with batch creation and location assignment
...
- **Purpose:** Get available locations for intake
...
##### Get Pending Intake (`poReceiving.getPendingReceiving`)

- **Purpose:** Get purchase orders pending intake (CONFIRMED, RECEIVING status)
...
##### Get Intake History (`poReceiving.getReceivingHistory`)

- **Purpose:** Get intake history for a PO
...
- **Purpose:** Confirm PO (supplier has confirmed receipt, changes status from SENT to CONFIRMED)
...
- **Purpose:** Submit PO to supplier (changes status from DRAFT to SENT)
...
##### Get By Supplier (`purchaseOrders.getByVendor`)

- **Purpose:** Get PO history for a supplier (DEPRECATED - use getBySupplier instead)
