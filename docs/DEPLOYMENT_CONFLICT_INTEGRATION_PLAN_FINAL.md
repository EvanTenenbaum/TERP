  ...
  await safeGit(async (git) => {
    ...
    try {
      ...
      if (errorMessage.includes('conflict') || errorMessage.includes('CONFLICT')) {
        console.log(chalk.yellow('⚠️  Rebase conflicts detected. Attempting auto-resolution...'));
        try {
          execSync('bash scripts/auto-resolve-conflicts.sh', { stdio: 'inherit' });
          await git.rebase(['--continue']);
        } catch (resolveError) {
          throw new Error('Auto-conflict resolution failed. Manual resolution required.');
        }
      } else {
        throw error;
      }
    ...
    await git.merge([branchName, '--no-ff', '-m', `feat: ${taskId} autonomous implementation`]);
      ...
      try {
        ...
        if (errorMessage.includes('failed to push') || errorMessage.includes('rejected')) {
          if (pushAttempts < maxPushAttempts) {
            console.log(chalk.yellow(`⚠️  Push rejected (attempt ${pushAttempts}/${maxPushAttempts}). Handling conflicts...`));
            // Run conflict handler
            try {
              execSync('bash scripts/handle-push-conflict.sh', { stdio: 'inherit' });
              // Retry push
              continue;
            } catch (conflictError) {
