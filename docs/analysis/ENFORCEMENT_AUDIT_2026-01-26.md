...
| No `suppliers` table              | "NEVER use"         | None        | 38 active uses             |
...
─────────────────────────────────────────────────────────
"NEVER use suppliers table"    →    38 active uses
...
// Would catch: suppliers., vendorId
```
...
# - suppliers table usage: grep -r "suppliers\." server/
...
| suppliers table usage | Yes         | No        | 38 violations      |
...
4. **Create custom ESLint rules** for:
   ...
   - `no-deprecated-table`: Ban `suppliers.`, `vendorId`
...
_Audit based on: .husky/_, .github/workflows/_, eslint.config.js, CLAUDE.md, and codebase grep analysis_
