...
| --------------------------------- | ---------------------------------------------------- |
| "NEVER use the `suppliers` table" | 38 active uses of `suppliers.` in server code        |
...
| "Soft deletes only"               | `db.delete(suppliers)` in production code            |
...
### 38 Active Uses of Deprecated `suppliers` Table

CLAUDE.md explicitly states:

> "**Never use the `suppliers` table** - Use `clients` with `isSeller=true`"

Yet a search reveals 38 active uses:

```typescript
// server/inventoryDb.ts
.leftJoin(suppliers, eq(lots.vendorId, suppliers.id))

// server/services/vendorMappingService.ts
const supplier = await db.query.suppliers.findFirst({

// server/routers/suppliers.ts (still exists!)
console.warn('[DEPRECATED] suppliers.getAll - use clients.list...');
```

### Why Deprecated Systems Persist

1. **Working code:** Existing code uses suppliers; it works; don't touch it
...
### The suppliers.ts Router Paradox

The router still exists and is actively used:

```typescript
// server/routers/suppliers.ts
console.warn("[DEPRECATED] suppliers.getAll...");
...
| -------------------------- | ---------------------------- |
| Don't use suppliers table  | None (just documentation)    |
...
| ---------------------------- | --------- | ----------------------- |
| suppliers → clients          | Yes       | No (38 uses remain)     |
...
Create task: "Remove all 38 suppliers table uses"
...
| suppliers table uses         | 38      | 0                     | 14 days     |
...
_Analysis based on full codebase audit, commit history, and protocol compliance review._
_Commit range: 4a1e4f2...9da49be + codebase grep analysis_
