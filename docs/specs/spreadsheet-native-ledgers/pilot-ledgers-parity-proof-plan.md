...
| `SALE-ORD-019` | sheet-native selection grammar                                 | `P0`        | Drag-range, Shift-range, Cmd discontiguous, and scope-selection proof                        | `partial`                  | March 18, 2026 staging proof on draft 618 confirmed document-mode focused selection plus Shift-range extension through the shared PowersheetGrid runtime after live row duplication. Queue drag-range, Cmd discontiguous selection, and row or column scope proof are still required.                                                            |
| `SALE-ORD-020` | sheet-native multi-cell editing                                | `P0`        | Multi-cell edit proof with pricing, validation, autosave, and undo                           | `partial`                  | March 18, 2026 staging proof on draft 618 confirmed live document-grid editing, row duplication, and range selection continue to run through the shared PowersheetGrid runtime without replacing Orders orchestration. Multi-cell clipboard, pricing recalculation, autosave, and undo proof remain.                                             |
...
| `SALE-ORD-024` | spreadsheet affordance visibility                              | `P0`        | Visible focus, range, selection summary, save state, and blocked feedback proof              | `partial`                  | March 18, 2026 staging proof on draft 618 confirmed document-mode selection summary, selected-row counts, running totals, and blocked invalid-edit feedback. Queue/support focus, save-state, and cross-surface visibility proof are still pending.                                                                                              |
| `SALE-ORD-025` | field-state visibility                                         | `P0`        | Visible locked, editable, and workflow-owned field-state proof                               | `partial`                  | March 18, 2026 staging proof on draft 618 confirmed document-mode locked product and batch cells remain distinct from editable quantity, COGS, and margin cells while blocked edits surface a clear rejection. Cross-surface proof is still required.                                                                                            |
...
| `SALE-ORD-032` | row insert / duplicate / delete parity                         | `P1`        | Document-grid row operation proof                                                            | `partial`                  | March 18, 2026 staging proof on draft 618 confirmed live duplicate-row behavior through the shared document-grid runtime, increasing the visible quantity row count from one to two. Delete-row and add-item row-operation proof are still required.                                                                                             |
...
| `SALE-ORD-035` | failure-mode spreadsheet proof bundle                          | `P0`        | Negative/trust proof for blocked fill, mixed paste, autosave undo, and hidden-row protection | `partial`                  | March 18, 2026 staging proof on draft 618 confirmed blocked invalid inline quantity edits now revert immediately to the last valid grid state while surfacing the rejection message, instead of leaking stale invalid values until reload. Mixed editable/locked paste, blocked fill, autosave undo, and hidden-row protection still need proof. |
