...
| FUL-024 | Concurrent edit detection         | Behavior   | P1          | Blocked — server mutations lack version input/check (DISC-FUL-005) |
...
| DISC-FUL-005 | Concurrent-edit detection inert end-to-end. Server pickPack mutations (packItems, markAllPacked, unpackItems, markOrderReady) do not accept `version` in Zod input schemas and do not check/increment `orders.version`. `getOrderDetails` does not return `version`. Pilot surface has no `useConcurrentEditDetection` hook. **Requires server-side changes first**: add `version` to input schemas, add optimistic-lock check + increment in each mutation, return `version` from queries. Client wiring can follow. | High — server-side prerequisite            |
