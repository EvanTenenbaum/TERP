...
**Goal:** a reusable, decisive UX strategy that makes TERP _faster than a spreadsheet_ while staying _safer than a spreadsheet_.
...
2. **Unify PO + Intake UX** via a _compressed_ “Direct Intake” experience, while keeping underlying business events distinct.
...
- Workflow compression: capture reality (“I got this from this supplier at this cost”) without navigating multiple modules.
...
**I want** a single continuous flow where I can input supplier + line items without modal interruptions,
...
2. Focus starts on Supplier.
3. User selects supplier → header fills defaults (terms/location).
...
- Supplier required to commit batch
...
- Header: supplier or category filter
