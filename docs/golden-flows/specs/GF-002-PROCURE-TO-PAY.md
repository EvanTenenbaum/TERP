...
The Procure-to-Pay (P2P) flow covers the complete purchasing cycle from creating purchase orders through intake of goods and recording supplier bills. This flow creates inventory batches and accounts payable entries.
...
- Navigate to PO intake workflow
...
| Pending POs List     | Cards with PO details      | Select to intake           |
| Intake Form          | Line items with qty inputs | Enter received qty, Submit |
...
**Recommended limit:** 50 items per intake session to avoid long transactions.
...
- **RULE-011**: Intake creates one lot per intake session
- **RULE-012**: Intake creates one batch per received line item
...
- **Touch Point**: Intake creates batches
...
| **No PO intake UI**                  | HIGH     | `poReceiving.*` endpoints have no frontend                                 | Build `POIntakeWorkSurface.tsx`           |
