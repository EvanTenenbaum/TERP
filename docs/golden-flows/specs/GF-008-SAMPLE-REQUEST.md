...
The Sample Request flow manages sales samples distributed to clients and prospects. Sales representatives create sample requests specifying products and quantities, which are then fulfilled from sample-designated inventory. The system tracks sample distribution, monthly allocation limits, conversions to sales, sample returns, and supplier returns. This flow provides ROI visibility into sample programs and ensures proper inventory tracking.
...
### Alternative Flow: Supplier Return (SAMPLE-007)
...
2. **Ship to Supplier** (VENDOR_RETURN_REQUESTED → SHIPPED_TO_VENDOR)
   - User clicks "Ship to Supplier" action
   - VendorShipDialog opens
   - Fields:
     - Tracking Number (required text input)
   - Calls `samples.shipToVendor`

3. **Confirm Supplier Return** (SHIPPED_TO_VENDOR → VENDOR_CONFIRMED)
   - User clicks "Confirm Supplier Received" action
...
### Supplier Return Workflow (SAMPLE-007)
...
| vendorShippedDate          | TIMESTAMP     | Yes      | When shipped to supplier                       |
| vendorConfirmedDate        | TIMESTAMP     | Yes      | When supplier confirmed                        |
...
8. **Supplier Return Prerequisites**
...
| "Sample must be returned or fulfilled to initiate supplier return" | requestVendorReturn on wrong status   | 400  | Complete sample return first              |
| "Sample is not in supplier return requested status"                | shipToVendor on wrong status          | 400  | Request supplier return first             |
| "Sample is not shipped to supplier"                                | confirmVendorReturn on wrong status   | 400  | Ship first                                |
...
- [ ] Complete supplier return workflow (request → ship → confirm)
...
- [ ] Full supplier return workflow via UI
