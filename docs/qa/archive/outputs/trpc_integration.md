...
| Type     | Procedure          | Purpose                                     |
| -------- | ------------------ | ------------------------------------------- |
| Query    | `suppliers.getAll` | ⚠️ **DEPRECATED** - Should use clients.list |
...
**Optimistic Updates**: ❌ None
**Critical Issue**: Uses deprecated suppliers.getAll endpoint
...
### Issue 2: Deprecated Supplier Router Usage

**Severity**: P1 - Critical
**Location**: DirectIntakeWorkSurface.tsx

**Problem**: Uses `trpc.suppliers.getAll()` which is deprecated. Should use `trpc.clients.list()` with `isSeller=true` filter.

**Impact**: Will break when supplier router is removed in future migration.

**Suggested Fix**:

```typescript
// Replace:
const vendorsQuery = trpc.suppliers.getAll.useQuery();
...
2. Migrate DirectIntakeWorkSurface from suppliers.getAll to clients.list
