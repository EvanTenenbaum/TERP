...
|                     | `/suppliers`                    | VendorsPage            | 16 flows      | MEDIUM      |
|                     | `/supplier-supply`              | VendorSupplyPage       | 12 flows      | LOW         |
...
2. **PO Intake** - Nested under Purchase Orders, easily missed
3. **Calendar Financials** - Hidden feature, no direct navigation
4. **Admin Data Augment** - Admin-only, not accessible to regular users
5. **Supplier Reminders** - Backend exists, no frontend implementation visible
...
10. Select supplier from dropdown
...
5. **Terminology Inconsistency Pattern**
   - "Supplier" vs "Seller" vs "Supplier"
...
_This report represents exhaustive analysis of the TERP codebase with systematic user flow generation across all pages, components, and API endpoints. Findings are based on code review, pattern analysis, and simulated user behavior modeling._
