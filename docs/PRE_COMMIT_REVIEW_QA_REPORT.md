...
try {
  ...
  console.warn(`Failed to parse AI response for ${file}: ${error instanceof Error ? error.message : String(error)}`);
...
const truncated = content.length > MAX_CONTEXT_LENGTH 
...
try {
  ...
  if (error instanceof Error && error.message.includes('EACCES')) {
    console.warn(`Cannot write to ${file}: permission denied. Skipping auto-fix.`);
...
const codeBlockMatch = longestBlock.match(/```(?:typescript|ts)?\n([\s\S]*?)```/);
```
