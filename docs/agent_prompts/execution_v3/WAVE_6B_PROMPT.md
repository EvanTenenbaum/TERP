...
export class QuickBooksClient {
  ...
  // Suppliers
  async createVendor(supplier: Supplier): Promise<any> {
    return new Promise((resolve, reject) => {
...
export class QuickBooksSyncService {
  ...
  async syncClient(clientId: number): Promise<void> {
    ...
    console.log(
      `[QB Sync] Client ${clientId} synced to QB customer ${qbCustomer.Id}`
    );
  }
  ...
  async syncInvoice(invoiceId: number): Promise<void> {
    ...
    const qbInvoice = await this.client.createInvoice(
      invoice,
      client.qbCustomerId!
    );
    ...
    console.log(
      `[QB Sync] Invoice ${invoiceId} synced to QB invoice ${qbInvoice.Id}`
    );
  }
  ...
  async syncPayment(paymentId: number): Promise<void> {
    ...
    console.log(
      `[QB Sync] Payment ${paymentId} synced to QB payment ${qbPayment.Id}`
    );
  }
  ...
  if (prefs.emailAll && user.email) {
    await sendEmail({
      ...
      html: renderTemplate("notification", {
        ...
        link: options.link
          ? `${process.env.APP_URL}${options.link}`
          : undefined,
      }),
    });
  }
