...
export const purchaseOrdersRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      logger.info("[PO] Creating purchase order", {
      ...
      const supplier = await db.query.suppliers.findFirst({
        where: eq(suppliers.id, input.vendorId),
      });
      if (!supplier) {
        throw new TRPCError({
          code: "NOT_FOUND",
...
## Task 2: Goods Intake (2.5 hours)

### Backend: Intake Router
...
export const receivingRouter = router({
  receiveGoods: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      logger.info("[Intake] Intake goods", {
      ...
      const po = await db.query.purchaseOrders.findFirst({
        ...
        with: { items: { with: { product: true } }, supplier: true },
      });
      ...
      if (!["confirmed", "shipped", "partial"].includes(po.status)) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Purchase order cannot be received",
        });
      }
      ...
      for (const item of input.items) {
        ...
        logger.info("[Intake] Batch created", {
          batchId: batch.id,
          batchCode,
          quantity: item.quantity,
        });
      ...
      logger.info("[Intake] Goods received", {
        poId: input.purchaseOrderId,
        newStatus,
...
export const photographyRouter = router({
  ...
  startSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (batch.status !== "awaiting_photography") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Batch not awaiting photography",
        });
      }
  ...
  uploadPhoto: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      logger.info("[Photography] Photo uploaded", {
        batchId: input.batchId,
        photoId: photo.id,
      });
  ...
  completeSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!hasPrimary) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "At least one primary photo is required",
        });
      }
      ...
      logger.info("[Photography] Session completed", {
        batchId: input.batchId,
...
export async function publishBatchToCatalog(
  ...
  if (existing) {
    ...
    logger.info("[Catalog] Catalog item updated", {
    ...
    logger.info("[Catalog] Batch published to catalog", {
      batchId,
  ...
  for (const item of activeCatalogItems) {
    ...
    if (availableQty !== item.availableQuantity) {
      ...
      if (availableQty <= 0) {
        ...
        logger.info("[Catalog] Auto-unpublished item with no quantity", {
...
# Goods intake
git add server/routers/intake.ts client/src/pages/Intake*.tsx
git commit -m "feat(INV-2): Implement goods intake with batch creation"
...
- Goods intake with batch creation
