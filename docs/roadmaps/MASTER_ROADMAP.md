...
## 📅 Parallel Sprint: Documentation & Testing Infrastructure (Jan 8-22, 2026)

**Strategic Focus:** Build documentation and testing infrastructure (runs parallel to Feature Flag Sprint)  
**Sprint Duration:** 2 weeks  
**Conflict Zone:** NONE - This sprint touches only docs/, tests/, and standalone modules
...
- `client/src/pages/AnalyticsPage.tsx`, `LeaderboardPage.tsx`
- `.github/workflows/*`
...
1. **API Documentation** - All 124 routers documented
2. **User Guide** - All 15 WS features documented for end users
3. **E2E Test Suite** - Playwright/Cypress tests for critical flows
4. **Unit Tests** - 80%+ coverage on standalone modules
5. **CI/CD** - Automated docs, coverage reporting, bundle monitoring

---

## 🔮 Future Sprint: Remaining Work (After Feature Flags)
...
**Spec Directory:** `docs/specs/performance-sprint/`  
**Total Estimate:** 64 hours  
**Dependencies:** None (can run in parallel with other sprints)

### Overview

This sprint addresses 7 critical performance bottlenecks identified in a comprehensive audit. The focus is on reducing API latency, minimizing memory usage, and improving scalability for large datasets.
...
PERF-006 (Cache) ─────────────────────────────────────────┐
PERF-001a (Limits) ───────────────────────────────────────┤
PERF-002a (Indexes) ──────────────────────────────────────┼──► Phase 1 Complete
PERF-007 (Startup) ───────────────────────────────────────┤
PERF-003a (Calendar Indexes) ─────────────────────────────┘
                                                          │
                                                          ▼
PERF-002 (Dashboard KPIs) ────────────────────────────────┐
PERF-001b (Virtualization) ───────────────────────────────┼──► Phase 2 Complete
PERF-004 (Full-Text Search) ──────────────────────────────┘
                                                          │
                                                          ▼
PERF-001c (Client Selectors) ─────────────────────────────┐
PERF-005 (Streaming Exports) ─────────────────────────────┴──► Phase 3 Complete
```
...
│                                                                               │
│  ┌──────────────────┐    ┌──────────────────┐                                │
│  │ UX & Stability   │    │ Performance      │  ← Can start immediately       │
│  │ Sprint (66h)     │    │ Sprint (64h)     │    (no dependencies)           │
│  │ STAB-*, ACT-*,   │    │ PERF-001 to      │                                │
│  │ ENH-*            │    │ PERF-007         │                                │
│  └──────────────────┘    └──────────────────┘                                │
│                                                                               │
│  ┌──────────────────┐                                                        │
