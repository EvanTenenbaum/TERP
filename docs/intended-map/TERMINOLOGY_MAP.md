...
| **Supplier**   | supplier (deprecated), seller | Client with `isSeller=true` who sells TO TERP    | CANONICAL_DICTIONARY.md |
| **Customer**   | buyer, client                 | Client who purchases FROM TERP                   | CANONICAL_DICTIONARY.md |
| **Supplier**   | -                             | **DEPRECATED** - Use Client with `isSeller=true` | CANONICAL_DICTIONARY.md |
...
| Supplier         | `suppliers`         | **DEPRECATED** - Do not use              |
...
| **Intake**     | purchase (deprecated) | Process of intake inventory                | GF-001:Terminology      |
...
| `VENDOR_RETURN_REQUESTED` | Supplier Return  | Going back to supplier |
...
| `vendorId` (in lots/brands) | `suppliers.id`                 | **LEGACY** - Migrate to `supplierClientId` |
