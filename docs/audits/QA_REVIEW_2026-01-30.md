...
**Status: ACKNOWLEDGED - Defenses Exist**

The user is correct that most unit tests mock the database. However, the system has multiple defenses:

| Defense                    | Location                                     | Purpose                               |
| -------------------------- | -------------------------------------------- | ------------------------------------- |
| Auto-migration             | `server/autoMigrate.ts`                      | Runs at startup, adds missing columns |
| Schema drift audit         | `scripts/audit/detect-schema-drift-v2.ts`    | Detects schema/DB mismatches          |
| Nightly CI check           | `.github/workflows/nightly-schema-check.yml` | Catches drift before deployment       |
| Graceful error handling    | `server/_core/trpc.ts`                       | Sanitizes SQL errors (SEC-042)        |
| Schema validation workflow | `.github/workflows/schema-validation.yml`    | Blocks deployments on critical drift  |

**Evidence of Handling**:

- `server/_core/trpc.test.ts:26` - Explicitly tests "Unknown column 'products.strainId'" scenario
- `server/routers/photography.test.ts:45` - Tests the strainId column error case
- Tests verify that errors are properly handled, not that they don't occur
...
### 3. "Backend verification ≠ Production readiness"
