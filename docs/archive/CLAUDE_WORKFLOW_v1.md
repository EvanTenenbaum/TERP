...
## 🔬 Live QA & The Testing Suite

To ensure the highest quality for TERP, we have a comprehensive **Testing Suite** that includes automated tests, manual QA protocols, and performance monitoring. The cornerstone of our manual QA process is the "live qa" command.

### Initiating a Live QA Session

When you want a thorough, human-like review of the entire production application, simply instruct the agent:

> "live qa"

This triggers a specialized QA agent that follows a rigorous, multi-phase testing protocol defined in the official QA prompt template.
...
You: "live qa"
     ↓
Agent loads QA prompt template
     ↓
Agent completes 8-step pre-flight check
     ↓
Agent creates QA session (tracked in ACTIVE_SESSIONS.md)
     ↓
Agent performs 7-layer systematic testing:
  1. Smoke Testing (does it load?)
  2. Functional Testing (do features work?)
  3. UI/UX Verification (does it match design system?)
  4. Data Integrity (is data displayed correctly?)
  5. Performance Testing (is it fast?)
  6. Error Handling & Security (are errors handled gracefully?)
  7. Regression Testing (are old bugs still fixed?)
     ↓
Agent logs every issue with screenshots
     ↓
Agent creates tasks in QA_TASKS_BACKLOG.md
     ↓
Agent delivers comprehensive QA report
```

**Template Location:** `docs/agent_prompts/live_qa/live_qa_prompt.md`

**Output Files:**
- `docs/sessions/completed/QA_REPORT_[Session-ID].md` - Detailed findings
- `docs/roadmaps/QA_TASKS_BACKLOG.md` - Updated with new bug tasks
- `qa_screenshots/[Session-ID]/` - Visual evidence for each issue
...
| Section | Content |
|---------|----------|
| **Summary** | Total issues found, priority distribution |
| **Issues** | Detailed bug reports with screenshots |
| **Reproduction Steps** | Exact steps to reproduce each issue |
| **Impact Analysis** | Business/user impact for each bug |
| **Estimated Effort** | Time to fix each issue |

This ensures that our manual QA is as structured and effective as our automated testing, providing a critical layer of quality assurance.
