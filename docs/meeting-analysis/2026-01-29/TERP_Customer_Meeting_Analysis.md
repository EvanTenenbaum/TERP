...
| **Meaning**             | The "DUAL" type designation in the client system indicates an entity that acts as both a supplier (supplier) and a buyer (customer).            |
| **Acceptance Criteria** | 1. Client type "DUAL" clearly labeled. 2. Tooltip or help text explains meaning. 3. Dual clients appear in both supplier and customer contexts. |
...
| **Meaning**             | The business workflow involves giving accounting staff (Z) a bulk cash amount, which they then allocate to individual payments. This is different from intake individual payments. |
...
### MEET-026: Intake Payments is Higher Risk - Owner Only
...
| **Domain**              | Payments                                                                                                                                                                        |
| **Priority**            | Now                                                                                                                                                                             |
| **Title**               | Payment intake restricted to owner/higher permission level                                                                                                                      |
| **Meaning**             | Intake payments from clients is considered a higher-risk activity that should be restricted to the owner, while accounting staff handles lower-risk supplier payments.          |
| **Acceptance Criteria** | 1. "Receive Payment" action requires elevated permissions. 2. Role-based access control for payment functions. 3. Accounting role can "Pay Supplier" but not "Receive Payment". |
| **Evidence**            | "she only needs like low risk farmers. I feel like intake payments is a little bit higher risk. Yep. That makes sense. Okay. That's good to know."                              |
| **Dependencies**        | None                                                                                                                                                                            |
...
| MEET-026 | Intake payments is higher risk     | Now      | Business Rule |
...
- Pick & Pack, Products, Inventory, Photography, Samples, Purchase Orders, Suppliers, Supplier Supply, Spreadsheet View, Direct Intake
...
_Analysis performed by: Manus AI_
