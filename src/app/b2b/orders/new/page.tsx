...
export default function NewB2BOrderPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const filtered = items.filter(it=> it.productId && it.unitCount>0)
      const res = await createB2BSale({ type, sourceId, targetId, items: filtered })
      if (res.success) {
      ...
      <form onSubmit={submit} className="bg-white shadow rounded-lg p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input className="w-full border rounded px-3 py-2" value={sourceId} onChange={e=>setSourceId(e.target.value)} placeholder="Customer/Vendor ID" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Target ID</label>
            <input className="w-full border rounded px-3 py-2" value={targetId} onChange={e=>setTargetId(e.target.value)} placeholder="Customer/Vendor ID" />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {items.map((it, idx)=> (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
              <input className="border rounded px-3 py-2" placeholder="Product ID" value={it.productId} onChange={e=>updateItem(idx,{productId:e.target.value})} />
