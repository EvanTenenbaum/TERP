...
export const vipPortalConfigurations = mysqlTable(
  "vip_portal_configurations",
  {
    ...
    moduleLiveCatalogEnabled: boolean("module_live_catalog_enabled")
      .default(false)
      .notNull(),
    moduleLiveShoppingEnabled: boolean("module_live_shopping_enabled")
      .default(true)
      .notNull(),
 ...
 */
export const clientCatalogViews = mysqlTable(
 ...
 */
export const clientInterestLists = mysqlTable(
  "client_interest_lists",
  {
    ...
    convertedBy: int("converted_by").references(() => users.id, {
      onDelete: "set null",
    }),
    notes: text("notes"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  ...
  table => ({
    clientIdIdx: index("idx_client_interest_lists_client_id").on(
      table.clientId
    ),
 ...
 */
export const clientInterestListItems = mysqlTable(
  "client_interest_list_items",
  {
    ...
    priceAtInterest: decimal("price_at_interest", {
      precision: 10,
      scale: 2,
    }).notNull(),
    quantityAtInterest: decimal("quantity_at_interest", {
      precision: 10,
      scale: 2,
    }),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  ...
  table => ({
    interestListIdIdx: index("idx_interest_list_items_list_id").on(
...
export type ClientInterestListItem =
  typeof clientInterestListItems.$inferSelect;
export type InsertClientInterestListItem =
  typeof clientInterestListItems.$inferInsert;

/**
 * Client Draft Interests
 * Temporary storage for items clients are considering (not yet submitted)
 */
export const clientDraftInterests = mysqlTable(
  ...
  table => ({
    clientIdIdx: index("idx_client_draft_interests_client_id").on(
      table.clientId
    ),
    batchIdIdx: index("idx_client_draft_interests_batch_id").on(table.batchId),
    uniqueClientBatch: unique("idx_client_draft_unique").on(
      table.clientId,
      table.batchId
    ),
  })
...
export type InsertClientDraftInterest =
  typeof clientDraftInterests.$inferInsert;

/**
 * Client Price Alerts
 * Stores price alert configurations for specific items
 */
export const clientPriceAlerts = mysqlTable(
  "client_price_alerts",
  {
    id: int("id").primaryKey().autoincrement(),
    clientId: int("client_id")
 ...
 */
export const adminImpersonationSessions = mysqlTable(
  ...
  table => ({
    adminUserIdIdx: index("idx_admin_imp_sessions_admin_user_id").on(
      table.adminUserId
    ),
...
export type InsertAdminImpersonationSession =
 ...
 */
export const adminImpersonationActions = mysqlTable(
  ...
  table => ({
    ...
    sessionFk: foreignKey({
      name: "fk_admin_imp_actions_session",
      columns: [table.sessionId],
      foreignColumns: [adminImpersonationSessions.id],
    }).onDelete("cascade"),
  })
);

export type AdminImpersonationAction =
  typeof adminImpersonationActions.$inferSelect;
export type InsertAdminImpersonationAction =
  typeof adminImpersonationActions.$inferInsert;
 ...
 */
export const vipTiers = mysqlTable(
  "vip_tiers",
  {
    id: int("id").primaryKey().autoincrement(),
    ...
    minSpendYtd: decimal("min_spend_ytd", { precision: 15, scale: 2 }).default(
      "0"
    ), // Minimum YTD spend
    ...
    minPaymentOnTimeRate: decimal("min_payment_on_time_rate", {
      precision: 5,
      scale: 2,
    }).default("0"), // 0-100%
    ...
    discountPercentage: decimal("discount_percentage", {
      precision: 5,
      scale: 2,
    }).default("0"), // Tier discount
    creditLimitMultiplier: decimal("credit_limit_multiplier", {
      precision: 5,
      scale: 2,
    }).default("1.00"), // Credit limit boost
 ...
 */
export const clientVipStatus = mysqlTable(
  "client_vip_status",
  {
    id: int("id").primaryKey().autoincrement(),
    clientId: int("client_id")
      ...
      .unique()
      .references(() => clients.id, { onDelete: "cascade" }),
    currentTierId: int("current_tier_id").references(() => vipTiers.id, {
      onDelete: "set null",
    }),
    ...
    paymentOnTimeRate: decimal("payment_on_time_rate", {
      precision: 5,
      scale: 2,
    }).default("100"), // 0-100%
    lifetimeSpend: decimal("lifetime_spend", {
      precision: 15,
      scale: 2,
    }).default("0"),
    ...
    overrideBy: int("override_by").references(() => users.id, {
      onDelete: "set null",
    }),
    ...
    nextTierProgress: decimal("next_tier_progress", {
      precision: 5,
      scale: 2,
    }).default("0"), // 0-100%
 ...
 */
export const vipTierHistory = mysqlTable(
  "vip_tier_history",
  {
    id: int("id").primaryKey().autoincrement(),
    clientId: int("client_id")
      .notNull()
      .references(() => clients.id, { onDelete: "cascade" }),
    previousTierId: int("previous_tier_id").references(() => vipTiers.id, {
      onDelete: "set null",
    }),
    newTierId: int("new_tier_id").references(() => vipTiers.id, {
      onDelete: "set null",
    }),
