...
-- ============================================================================
-- 2. Add Database Indexes for Performance (Production Schema)
-- ============================================================================

-- Check and create indexes only if they don't exist
-- Note: MySQL doesn't support CREATE INDEX IF NOT EXISTS, so we use a different approach

-- Batches table indexes (using production column names)
CREATE INDEX `idx_batches_status` ON `batches` (`batchStatus`);
...
CREATE INDEX `idx_products_strain_id` ON `products` (`strainId`);

-- Lots table indexes
CREATE INDEX `idx_lots_vendor_id` ON `lots` (`vendorId`);
CREATE INDEX `idx_lots_date` ON `lots` (`date`);

-- Vendors table indexes
CREATE INDEX `idx_vendors_name` ON `vendors` (`name`);

-- Brands table indexes
CREATE INDEX `idx_brands_vendor_id` ON `brands` (`vendorId`);
