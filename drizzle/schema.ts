...
export const calendarEvents = mysqlTable("calendar_events", {
  ...
  dateRangeIdx: index("idx_calendar_date_range").on(table.startDate, table.endDate),
  moduleIdx: index("idx_calendar_module").on(table.module),
  entityIdx: index("idx_calendar_entity").on(table.entityType, table.entityId),
...
export const calendarRecurrenceRules = mysqlTable("calendar_recurrence_rules", {
  id: int("id").autoincrement().primaryKey(),
  eventId: int("event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }).unique(),
  ...
  byDayOfWeekInMonth: json("by_day_of_week_in_month").$type<{week: number, day: number}[]>(), // e.g., "2nd Tuesday"
...
export type CalendarRecurrenceRule = typeof calendarRecurrenceRules.$inferSelect;
export type InsertCalendarRecurrenceRule = typeof calendarRecurrenceRules.$inferInsert;
 ...
 */
export const calendarRecurrenceInstances = mysqlTable("calendar_recurrence_instances", {
  id: int("id").autoincrement().primaryKey(),
  parentEventId: int("parent_event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
  ...
  parentDateIdx: index("idx_instance_parent_date").on(table.parentEventId, table.instanceDate),
  dateRangeIdx: index("idx_instance_date_range").on(table.instanceDate, table.startTime),
  statusIdx: index("idx_instance_status").on(table.status),
}));

export type CalendarRecurrenceInstance = typeof calendarRecurrenceInstances.$inferSelect;
export type InsertCalendarRecurrenceInstance = typeof calendarRecurrenceInstances.$inferInsert;

/**
 * Calendar Event Participants table
 * Multi-user event support with RSVP functionality
 */
export const calendarEventParticipants = mysqlTable("calendar_event_participants", {
  id: int("id").autoincrement().primaryKey(),
  eventId: int("event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
  userId: int("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  
  // Participation details
  role: mysqlEnum("role", ["ORGANIZER", "REQUIRED", "OPTIONAL", "OBSERVER"]).default("REQUIRED").notNull(),
  responseStatus: mysqlEnum("response_status", ["PENDING", "ACCEPTED", "DECLINED", "TENTATIVE"]).default("PENDING").notNull(),
  ...
  eventUserIdx: unique("idx_participant_event_user").on(table.eventId, table.userId),
...
export type InsertCalendarEventParticipant = typeof calendarEventParticipants.$inferInsert;

/**
 * Calendar Reminders table
 * Event reminders with delivery tracking
 */
export const calendarReminders = mysqlTable("calendar_reminders", {
  id: int("id").autoincrement().primaryKey(),
  eventId: int("event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
  userId: int("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
 ...
 */
export const calendarEventHistory = mysqlTable("calendar_event_history", {
  id: int("id").autoincrement().primaryKey(),
  eventId: int("event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
  
  // Change tracking
  changeType: mysqlEnum("change_type", ["CREATED", "UPDATED", "DELETED", "RESCHEDULED", "CANCELLED", "COMPLETED"]).notNull(),
...
export type CalendarEventHistoryEntry = typeof calendarEventHistory.$inferSelect;
export type InsertCalendarEventHistoryEntry = typeof calendarEventHistory.$inferInsert;

/**
 * Calendar Event Attachments table
 * File attachments for events
 */
export const calendarEventAttachments = mysqlTable("calendar_event_attachments", {
  id: int("id").autoincrement().primaryKey(),
  eventId: int("event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
...
export type CalendarEventAttachment = typeof calendarEventAttachments.$inferSelect;
export type InsertCalendarEventAttachment = typeof calendarEventAttachments.$inferInsert;

/**
 * Calendar Views table
 * User-specific calendar view configurations
 */
export const calendarViews = mysqlTable("calendar_views", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  // Display preferences
  defaultViewType: mysqlEnum("default_view_type", ["MONTH", "WEEK", "DAY", "AGENDA"]).default("MONTH").notNull(),
  ...
  userDefaultIdx: index("idx_view_user_default").on(table.userId, table.isDefault),
}));
 ...
 */
export const calendarEventPermissions = mysqlTable("calendar_event_permissions", {
  id: int("id").autoincrement().primaryKey(),
  eventId: int("event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
  ...
  eventGranteeIdx: index("idx_permission_event_grantee").on(table.eventId, table.grantType, table.granteeId),
}));

export type CalendarEventPermission = typeof calendarEventPermissions.$inferSelect;
export type InsertCalendarEventPermission = typeof calendarEventPermissions.$inferInsert;
...
export const clientMeetingHistory = mysqlTable("client_meeting_history", {
  ...
  calendarEventId: int("calendar_event_id").notNull().references(() => calendarEvents.id, { onDelete: "cascade" }),
  meetingDate: timestamp("meeting_date").notNull(),
  meetingType: varchar("meeting_type", { length: 100 }).notNull(), // 'sales', 'collections', 'review'
  attendees: json("attendees").$type<Array<{ userId?: number; contactId?: number; name: string }>>(), // Array of user IDs and client contact IDs
  outcome: varchar("outcome", { length: 50 }).notNull(), // 'completed', 'no-show', 'rescheduled', 'cancelled'
  notes: text("notes"),
  actionItems: json("action_items").$type<Array<{ text: string; completed: boolean; assignedTo?: number }>>(), // Array of action items
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
...
export type ClientMeetingHistoryEntry = typeof clientMeetingHistory.$inferSelect;
export type InsertClientMeetingHistoryEntry = typeof clientMeetingHistory.$inferInsert;
