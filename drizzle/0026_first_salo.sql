...
CREATE TABLE `leaderboard_default_weights` (
	`id` int AUTO_INCREMENT NOT NULL,
	`leaderboard_client_type` enum('CUSTOMER','SUPPLIER','ALL') NOT NULL DEFAULT 'ALL',
	`weights` json NOT NULL,
	`updated_by` int,
	`created_at` timestamp NOT NULL DEFAULT (now()),
	`updated_at` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `leaderboard_default_weights_id` PRIMARY KEY(`id`),
	CONSTRAINT `idx_default_weights_client_type` UNIQUE(`leaderboard_client_type`)
);
--> statement-breakpoint
...
CREATE TABLE `leaderboard_weight_configs` (
	...
	CONSTRAINT `idx_user_config_type` UNIQUE(`user_id`,`config_name`,`leaderboard_client_type`)
);
--> statement-breakpoint
ALTER TABLE `dashboard_widget_configs` ADD CONSTRAINT `dashboard_widget_configs_user_id_users_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
