#!/usr/bin/env tsx
/**
 * Programmatic Prompt Generator
 *
 * Generates consistent, protocol-compliant task prompts from roadmap data.
 * Ensures all prompts include roadmap update steps and push-to-main workflow.
 */

import fs from "fs";
import path from "path";

interface Task {
  id: string;
  title: string;
  priority: string;
  effort: string;
  description: string;
  module?: string;
  dependencies?: string;
}

const REPO_URL = "https://github.com/EvanTenenbaum/TERP";
const ROADMAP_GUIDE = "docs/ROADMAP_AGENT_GUIDE.md";

/**
 * Generate a complete task prompt from task data
 */
function generatePrompt(task: Task): string {
  const today = new Date().toISOString().split("T")[0];

  return `# ${task.id}: ${task.title}

**Repository:** ${REPO_URL}  
**Task ID:** ${task.id}  
**Priority:** ${task.priority}  
**Estimated Time:** ${task.effort}  
**Module:** ${task.module || "Multiple modules"}  
**Dependencies:** ${task.dependencies || "None"}

---

## 📋 Task Description

${task.description}

---

## 🚀 4-Phase Execution Protocol

Follow this protocol exactly. See \`${ROADMAP_GUIDE}\` for complete details.

### Phase 1: Pre-Flight Check (5-10 minutes)

**Objective:** Register your session and verify no conflicts.

**Steps:**

1. **Clone the repository:**
   \`\`\`bash
   git clone ${REPO_URL}.git
   cd TERP
   pnpm install
   \`\`\`

2. **Read the roadmap:**
   \`\`\`bash
   cat docs/roadmaps/MASTER_ROADMAP.md | grep -A 20 "${task.id}"
   \`\`\`

3. **Check for conflicts:**
   \`\`\`bash
   cat docs/ACTIVE_SESSIONS.md
   \`\`\`
   
   If another agent is working on the same module, STOP and coordinate.

4. **Create session file:**
   \`\`\`bash
   SESSION_ID="Session-$(date +%Y%m%d)-${task.id}-$(openssl rand -hex 4)"
   cat > "docs/sessions/active/\${SESSION_ID}.md" << 'EOF'
# ${task.id}: ${task.title}

**Agent:** [Your name/ID]
**Started:** $(date +%Y-%m-%d)
**Status:** In Progress

## Progress
- [ ] Phase 1: Pre-flight check complete
- [ ] Phase 2: Implementation complete
- [ ] Phase 3: Testing complete
- [ ] Phase 4: Roadmap updated and pushed

## Notes
[Add notes here as you work]
EOF
   \`\`\`

5. **Register session atomically:**
   \`\`\`bash
   git pull origin main
   echo "- ${task.id}: \${SESSION_ID} ($(date +%Y-%m-%d))" >> docs/ACTIVE_SESSIONS.md
   git add docs/ACTIVE_SESSIONS.md docs/sessions/active/\${SESSION_ID}.md
   git commit -m "Register session for ${task.id}"
   git push origin main
   \`\`\`
   
   **If push fails:** Another agent registered first. Pull, check conflicts, try again.

---

### Phase 2: Implementation (${task.effort})

**Objective:** Complete the task following TDD principles.

**Steps:**

1. **Create feature branch:**
   \`\`\`bash
   git checkout -b ${task.id.toLowerCase()}-fix
   \`\`\`

2. **Implement the solution:**
   - Write tests first (TDD)
   - Implement the fix
   - Ensure all tests pass
   - Follow existing code patterns

3. **Test thoroughly:**
   \`\`\`bash
   pnpm test
   pnpm check
   \`\`\`

4. **Commit frequently:**
   \`\`\`bash
   git add -A
   git commit -m "${task.id}: [Description of change]"
   \`\`\`

---

### Phase 3: Verification (15-30 minutes)

**Objective:** Ensure quality and completeness.

**Checklist:**

- [ ] All tests passing (\`pnpm test\`)
- [ ] Zero TypeScript errors (\`pnpm check\`)
- [ ] Zero ESLint warnings
- [ ] Code follows project conventions
- [ ] Changes are well-documented
- [ ] No console.log or debug code
- [ ] Performance is acceptable

**Create completion report:**

\`\`\`bash
cat > "docs/${task.id}-COMPLETION-REPORT.md" << 'EOF'
# ${task.id} Completion Report

**Task:** ${task.title}
**Completed:** $(date +%Y-%m-%d)
**Agent:** [Your name/ID]

## Summary

[Brief summary of what was done]

## Changes Made

- [List key changes]

## Testing

- [Describe testing performed]

## Notes

- [Any important notes or caveats]
EOF
\`\`\`

---

### Phase 4: Completion (10-15 minutes)

**Objective:** Update roadmap and push to main.

**Steps:**

1. **Update roadmap status:**
   
   Edit \`docs/roadmaps/MASTER_ROADMAP.md\` and find your task:
   
   **Change from:**
   \`\`\`markdown
   **Priority:** ${task.priority} | **Status:** Not Started | **Effort:** ${task.effort}
   \`\`\`
   
   **Change to:**
   \`\`\`markdown
   **Priority:** ${task.priority} | **Status:** ✅ Complete (${today}) | **Effort:** ${task.effort}
   
   **Resolution:** [Brief summary of what was done]
   See docs/${task.id}-COMPLETION-REPORT.md for details.
   \`\`\`

2. **Archive session file:**
   \`\`\`bash
   mv docs/sessions/active/\${SESSION_ID}.md docs/sessions/completed/
   \`\`\`

3. **Update ACTIVE_SESSIONS.md:**
   
   Remove your session entry from \`docs/ACTIVE_SESSIONS.md\`.

4. **Commit all changes:**
   \`\`\`bash
   git add -A
   git commit -m "Complete ${task.id}: ${task.title}

- [Brief description of changes]
- All tests passing
- Roadmap updated
- Session archived"
   \`\`\`

5. **Push directly to main:**
   \`\`\`bash
   git push origin ${task.id.toLowerCase()}-fix:main
   \`\`\`
   
   **DO NOT create a pull request.** Push directly to main.

6. **Verify deployment:**
   
   Check that your changes are live on main and deployment succeeded.

---

## ✅ Success Criteria

- [ ] Task implementation complete
- [ ] All tests passing
- [ ] Zero errors or warnings
- [ ] Completion report created
- [ ] Roadmap updated to "✅ Complete"
- [ ] Session archived
- [ ] Changes pushed to main
- [ ] Deployment successful

---

## 📚 Additional Resources

- **Roadmap Agent Guide:** \`${ROADMAP_GUIDE}\`
- **Master Roadmap:** \`docs/roadmaps/MASTER_ROADMAP.md\`
- **Session Template:** \`docs/templates/SESSION_TEMPLATE.md\`
- **Completion Template:** \`docs/templates/COMPLETION_REPORT_TEMPLATE.md\`

---

**Generated:** ${today}  
**Generator:** scripts/generate-prompts.ts
`;
}

/**
 * Parse tasks from the QA tasks backlog
 */
function parseQATasks(): Task[] {
  const backlogPath = path.join(
    process.cwd(),
    "docs/roadmaps/QA_TASKS_BACKLOG.md"
  );
  const content = fs.readFileSync(backlogPath, "utf-8");

  const tasks: Task[] = [];
  const taskRegex =
    /### (QA-\d+): (.+?)\n[\s\S]*?\*\*Priority:\*\* (P\d+)[\s\S]*?\*\*Description:\*\*\s*\n([\s\S]*?)\n\*\*(?:Impact|Acceptance)[\s\S]*?\*\*Estimated Effort:\*\* ([\d-]+ hours?)/g;

  let match;
  while ((match = taskRegex.exec(content)) !== null) {
    const [, id, title, priority, description, effort] = match;
    tasks.push({
      id,
      title: title.trim(),
      priority,
      effort,
      description: description.trim(),
    });
  }

  return tasks;
}

/**
 * Main execution
 */
function main() {
  console.log("🚀 Generating task prompts...\n");

  const tasks = parseQATasks();
  console.log(`Found ${tasks.length} QA tasks\n`);

  const promptsDir = path.join(process.cwd(), "docs/prompts");

  let generated = 0;
  const skipped = 0;

  for (const task of tasks) {
    const promptPath = path.join(promptsDir, `${task.id}.md`);
    const prompt = generatePrompt(task);

    fs.writeFileSync(promptPath, prompt, "utf-8");
    console.log(`✅ Generated: ${task.id}.md`);
    generated++;
  }

  console.log(`\n✨ Complete!`);
  console.log(`   Generated: ${generated} prompts`);
  console.log(`   Skipped: ${skipped} prompts`);
  console.log(`\n📁 Prompts saved to: docs/prompts/`);
}

main();
