...
function main(): void {
  ...
  const expectedUnresolvedRoles = [...INTENTIONALLY_UNRESOLVED_MATRIX_ROLES].sort();
  ...
  assertCondition(
    resolveRoleFixtureLabel(purchasingRow, "Purchasing") === "Inventory Manager",
  ...
  const primaryPacket = buildGeneratedFlows(rows, chainSummaries, oracleSummaries, {
    count: 24,
    seed: 20260327,
    anchors: 8,
    domains: null,
  ...
  assertCondition(primaryPacket.candidateRows > 0, "No client-wired rows were loaded.");
  assertCondition(
  ...
  const noProbePacket = buildGeneratedFlows(rows, chainSummaries, oracleSummaries, {
    count: 24,
    seed: 20260327,
    anchors: 8,
    domains: null,
  ...
  assertCondition(
    noProbePacket.flows.every(flow => flow.mistakePattern !== "Permission probe"),
  ...
  console.log("Confused human flow check passed.");
  console.log(
