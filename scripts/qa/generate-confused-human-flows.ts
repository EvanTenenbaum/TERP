...
> = {
  ...
  "All Authenticated Users": row =>
    DOMAIN_QA_ROLE_HINTS[row.domain] ?? "Sales Manager",
};
...
const PERSONA_LENSES: PersonaLens[] = [
  {
    ...
    description:
      "Still learning labels and likely to pick the wrong obvious action first.",
  },
  {
    id: "rushed-operator",
    label: "Rushed operator",
    description:
      "Moves fast, skim-reads, and assumes the app kept the last context.",
  },
  {
    id: "cautious-operator",
    label: "Cautious operator",
    description:
      "Double-checks before committing and backs out when feedback feels unclear.",
  },
  {
    id: "distracted-multitasker",
    label: "Distracted multitasker",
    description:
      "Switches tabs, loses context, and returns expecting the UI to remember state.",
  },
  {
    id: "returning-user",
    label: "Returning user",
    description:
      "Assumes old habits still work and may miss changed labels or hidden filters.",
  },
  {
    id: "keyboard-first",
...
const MISTAKE_PATTERNS: MistakePattern[] = [
  {
    ...
    description:
      "Open the wrong row, tab, or action first before correcting course.",
    recoveryCheck:
      "Verify the app makes it obvious what changed and allows an easy recovery.",
  },
  {
    ...
    description:
      "Apply a partial filter, forget about it, and continue as if the list is complete.",
    recoveryCheck:
      "Check whether the UI exposes the active filter clearly and avoids false empty states.",
  },
  {
    ...
    description:
      "Use an incomplete or slightly wrong search term, then misread the results.",
    recoveryCheck:
      "Look for helpful empty states, fuzzy matches, or clear recovery guidance.",
  },
  {
    ...
    description:
      "Open the flow, cancel midway, then come back expecting clean state.",
    recoveryCheck:
      "Confirm canceled work does not leak state or leave ghost selections behind.",
  },
  {
    ...
    description:
      "Use browser back/forward after opening a drawer, detail panel, or cross-link.",
    recoveryCheck:
      "Verify URLs, selection state, and detail panels stay truthful after backtracking.",
  },
  {
    id: "refresh-mid-flow",
    ...
    recoveryCheck:
      "Check whether the page recovers cleanly and explains any lost draft or retained state.",
  },
  {
    ...
    description:
      "Click confirm twice or retry quickly because the UI feels slow or ambiguous.",
    recoveryCheck:
      "Look for duplicate submissions, repeated toasts, or unclear disabled/loading states.",
  },
  {
    ...
    description:
      "Jump to a nearby page, then return expecting the original selection or filter to persist sensibly.",
    recoveryCheck:
      "Verify context persistence is intentional and not silently misleading.",
  },
  {
    ...
    description:
      "Choose the most obvious action based on label alone, even if it means the wrong workflow.",
    recoveryCheck:
      "Check whether button labels and resulting destinations align to normal human expectations.",
  },
  {
    ...
    description:
      "Run the flow on a narrow viewport where filters or actions may be tucked away.",
    recoveryCheck:
      "Verify the next action remains discoverable and that overlays do not trap the user.",
  },
  {
    id: "permission-probe",
    ...
    description:
      "Attempt the flow as a plausible but wrong role to see whether the UI fails safely.",
    recoveryCheck:
      "Verify blocked roles are denied clearly without broken or half-rendered UI.",
...
function parseArgs(argv: string[]): Options {
  ...
  while (args.length > 0) {
    ...
    switch (flag) {
      ...
      case "--domains":
        if (!nextValue)
          throw new Error("--domains requires a comma-separated value");
...
function normalizeToken(value: string): string[] {
  return value
    ...
    .replace(/[^a-z0-9/]+/g, " ")
    .split(/\s+/)
    .filter(token => token.length >= 3 || ALLOWED_SHORT_TOKENS.has(token));
}
...
function scoreChain(row: MatrixRow, summary: ChainSummary): number {
  ...
  if (
    ...
    summary.crudTags.includes("crud:create")
  ) {
    score += 2;
  }
  if (
    archetype.includes("update") &&
    summary.crudTags.includes("crud:update")
  ) {
    score += 2;
  }
  if (
...
function getDeviceChoices(
  archetype: string
): Array<{ item: "desktop" | "tablet" | "mobile"; weight: number }> {
...
function chooseRole(
  ...
  if (allowedRoles.length === 0) {
    if (!warnedMissingRoleFixtures.has(row.key)) {
      const unresolvedRoles = row.roles.filter(
        role => !resolveRoleFixture(row, role)
      );
      const unresolvedSuffix =
        unresolvedRoles.length > 0 ? ` [${unresolvedRoles.join(", ")}]` : "";
      const intentNote =
        unresolvedRoles.length > 0 &&
        unresolvedRoles.every(role =>
          INTENTIONALLY_UNRESOLVED_MATRIX_ROLES.has(role)
        )
...
function buildGeneratedFlows(
  ...
  const flows = selected.map((candidate, index) => {
    ...
    if (
      ...
      mistakePattern = pickMistakePattern(
        candidate.row,
        new SeededRng(
          hashSeed(`${options.seed}:${candidate.row.key}:fallback-mistake`)
        ),
        false
      );
      roleChoice = chooseRole(
        ...
        mistakePattern,
        new SeededRng(
          hashSeed(`${options.seed}:${candidate.row.key}:fallback-role`)
        )
      );
    }
    ...
    return {
      ...
      linkedOracleFlowIds: candidate.support.linkedOracles.map(
        match => match.id
      ),
  ...
  return {
    ...
    summary: {
      ...
      permissionProbes: flows.filter(
        flow => flow.roleMode === "permission-probe"
      ).length,
    },
    flows,
  };
...
function printSummary(packet: OutputPacket): void {
  console.info(`Seed: ${packet.seed}`);
  console.info(`Candidate rows: ${packet.candidateRows}`);
  console.info(`Generated runs: ${packet.selectedCount}`);
  console.info(`Anchors: ${packet.anchors}`);
  console.info(`Executors: ${formatCounts(packet.summary.byExecutor)}`);
  console.info(`Priority: ${formatCounts(packet.summary.byPriority)}`);
  console.info(`Devices: ${formatCounts(packet.summary.byDevice)}`);
  console.info(`Mistakes: ${formatCounts(packet.summary.byMistakePattern)}`);
  console.info("");
  console.info("Sample runs:");
...
function main(): void {
  ...
  const { rows, availableDomains, knownDomains } = loadMatrixRows(
    options.domains
  );
  if (rows.length === 0) {
    const scopedMessage =
  ...
  if (options.output) {
    console.info(`Wrote ${options.format} packet to ${options.output}`);
    console.info("");
  }
