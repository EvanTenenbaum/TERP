...
const ROADMAP_PATH = join(process.cwd(), 'docs/roadmaps/MASTER_ROADMAP.md');
...
function getSwarmStatus(): SwarmStatus {
  ...
  if (existsSync(SESSIONS_DIR)) {
    ...
    for (const file of sessionFiles) {
      ...
      const taskMatch = file.match(/(BUG|ST|RF|WF|DATA|INFRA|QA|CL|FEATURE)-\d+/i);
      ...
      try {
        ...
        if (branchMatch) {
          ...
          const agentBranch = gitBranches.split('\n').find(b => b.includes(`agent/${taskId}`));
          if (agentBranch) {
  ...
  try {
    const statusOutput = execSync('pnpm swarm status 2>&1', { encoding: 'utf-8' });
  ...
  if (existsSync(completedDir)) {
    ...
    const completedFiles = readdirSync(completedDir).filter(f => 
      f.includes(today) && f.endsWith('.md')
    );
...
function formatStatusReport(status: SwarmStatus, format: 'text' | 'json' = 'text'): string {
  if (format === 'json') {
...
function main() {
  ...
  const webhookUrl = webhookIndex >= 0 && args[webhookIndex + 1] 
    ? args[webhookIndex + 1] 
