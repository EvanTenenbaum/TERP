...
async function getVendorsWithoutCollision(collisionVendorIds: number[]): Promise<number> {
  try {
...
async function main() {
  ...
  for (const collision of [...exactMatches, ...caseInsensitiveMatches, ...trimmedMatches]) {
  ...
  const vendorsWithoutCollision = await getVendorsWithoutCollision(collisionVendorIds);
  ...
  const csvHeader = 'vendor_id,vendor_name,client_id,client_name,client_teri_code,match_type,recommendation\n';
  const csvRows = allCollisions.map(c => 
  ...
  if (allCollisions.length === 0) {
    ...
    console.log(`  - Case-insensitive: ${caseInsensitiveMatches.length} (recommend: manual review)`);
    console.log(`  - Trimmed: ${trimmedMatches.length} (recommend: manual review)`);
    console.log('');
    console.log(`Vendors without collision: ${vendorsWithoutCollision} (can migrate directly)`);
    console.log('');
    console.log('⚠️  IMPORTANT: Review collisions before migration!');
    console.log('   - "merge" = Same entity, merge vendor into existing client');
    console.log('   - "rename" = Different entity, rename vendor before migration');
