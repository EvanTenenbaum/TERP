...
interface DriftIssue {
  type: 'missing_column' | 'extra_column' | 'type_mismatch' | 'missing_fk' | 'missing_index' | 'naming_inconsistency';
...
const EXPECTED_FK_COLUMNS: Record<string, { column: string; referencedTable: string }[]> = {
  invoices: [
  ...
  audit_logs: [
    { column: 'actorId', referencedTable: 'users' },
  ],
  product_media: [
    { column: 'productId', referencedTable: 'products' },
...
function checkMissingFKs(
  ...
  for (const expected of expectedFKs) {
    ...
    // Check if FK exists
    const fkExists = existingFKs.some(fk => 
...
async function analyzeTable(tableName: string): Promise<TableDrift> {
  const columns = await getTableColumns(db as Parameters<typeof getTableColumns>[0], tableName);
  const fks = await getForeignKeys(db as Parameters<typeof getForeignKeys>[0], tableName);
  const indexes = await getIndexes(db as Parameters<typeof getIndexes>[0], tableName);
...
async function main() {
  ...
  const tables = await getTableList(db as Parameters<typeof getTableList>[0]);
