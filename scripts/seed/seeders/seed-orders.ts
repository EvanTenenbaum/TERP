...
function generateOrderItems(
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    const marginPercent = faker.number.float({
      min: 15,
      max: 35,
      fractionDigits: 2,
    });
...
function generateOrder(
  ...
  const { items, subtotal, totalCogs } = generateOrderItems(
    availableBatches,
    itemCount
  );
  ...
  const paymentTerms = isConsignment
    ? "CONSIGNMENT"
    : faker.helpers.arrayElement(["NET_7", "NET_15", "NET_30", "COD"]);
  ...
  return {
    ...
    validUntil: null,
    quoteStatus: isDraft ? "UNSENT" : null, // Requirements 5.1, 5.2
    paymentTerms,
    cashPayment: "0.00",
    dueDate,
    saleStatus,
...
export async function seedOrders(
  ...
  return withPerformanceLogging("seed:orders", async () => {
    try {
      ...
      const whaleClients = existingClients.filter(c =>
        c.teriCode?.startsWith("WHL")
      );
      const regularClients = existingClients.filter(
        c => !c.teriCode?.startsWith("WHL") && !c.teriCode?.startsWith("VND")
      );
      ...
      if (!hasProducts) {
        ...
        result.errors.push(
          "Warning: No products found. Order items will have limited metadata."
        );
      }
      ...
      const existingBatches: BatchWithMetadata[] = batchesWithMetadata.map(
        b => ({
          id: b.id,
      ...
      for (let i = 0; i < draftCount; i++) {
        const clientPool =
          regularClients.length > 0 ? regularClients : existingClients;
        const client = clientPool[i % clientPool.length];
